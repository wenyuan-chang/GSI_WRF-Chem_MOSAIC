...
cat <<EOF > gsiparm.anl
 ...
 &SETUP
   miter=10,niter(1)=50,niter(2)=50,
   write_diag(1)=.true.,write_diag(2)=.false.,write_diag(3)=.true.,
   gencode=78,qoption=1,
   factqmin=0.0,factqmax=0.0,

!!! cwy ---------------------------------------- cwy step 3 for mosaic
...
!!! cwy ---------------------------------------- cwy step 3 for mosaic
 ...
 /
 &OBS_INPUT
   dmesh(1)=120.0,dmesh(2)=60.0,dmesh(3)=30,time_window_max=3,ext_sonde=.true.,
 /
OBS_INPUT::
...
::
 ...
 &CHEM
  ...
  laeroana_mosaic=${bk_laeroana_mosaic},
 ...
 &SINGLEOB_TEST
   ...
   oblat=38.,oblon=270.,obpres=500.,obdattim=${ANAL_TIME},
   obhourset=0.,
 /
EOF
