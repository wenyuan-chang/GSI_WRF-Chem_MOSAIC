...
do ii=1,nvars
   ...
   if(trim(adjustl(source))=='motley') then
      ...
      if(ilev==1) then
         ...
         !!! cwy mosaic ----------------------------------
         if( ii==1 ) c3var=var(1:3)
         if( index(var,c3var)/=0 ) n_cvaer=n_cvaer+1
         if( index(var,'mas')/=0 ) n_cvmas=1
         if( index(var,'num')/=0 ) n_cvnum=1
         !!! cwy mosaic ----------------------------------
      endif
   endif
...
!!! cwy mosaic ----------------------------------
if( n_cvmas/=0 ) then; n_cvmas=n_cvaer; n_cvaer=0; nsize_mosaic_wrf=n_cvmas; end if
if( n_cvnum/=0 ) then; n_cvnum=n_cvaer; n_cvaer=0; nsize_mosaic_wrf=n_cvnum; end if
if( n_cvmas==0 .and. n_cvnum==0 ) nsize_mosaic_wrf=n_cvaer
print *, 'cwy ==========================='
print '(4(a9,i))', '   nsize=', nsize_mosaic_wrf, ' n_cvmas=', n_cvmas, ' n_cvnum=', n_cvnum, ' n_cvaer=', n_cvaer
if( nsize_mosaic_wrf==0 ) print *, 'NOTE: nsize_mosaic_wrf is only possible when running mete DA'
print *, 'cwy ==========================='
!!! cwy mosaic ----------------------------------
