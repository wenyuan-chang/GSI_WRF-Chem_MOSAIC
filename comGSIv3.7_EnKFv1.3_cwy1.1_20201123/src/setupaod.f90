...
!       Interpolate model fields to observation location, call crtm and create jacobians
        ...
        call call_aod_mosaic(dtime,data_s(:,n),nchanl,nreal,layer_od,zhgt,jacobian_aero,'modis_aod')
        else
...
!!! cwy mosaic ------------------------------------------------------------------------ chem
                if( laeroana_mosaic ) then
                  ...
                  tnoise(i)=sf_aodmodis*tnoise(i)/sum(zhgt(:))*1.e+6
