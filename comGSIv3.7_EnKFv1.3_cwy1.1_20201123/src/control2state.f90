...
character(len=3), parameter :: mycvars(ncvars) = (/  &  ! vars from CV needed here
                'sf ', 'vp ', 'ps ', 't  ', 'q  ', 'cw ', 'ql ', 'qi ', 'w  ' /) !!! here, q means RH
...
character(len=4), parameter :: mysvars(nsvars) = (/  &  ! vars from ST needed here
                'u   ', 'v   ', 'prse', 'q   ', 'tsen', 'ql  ', 'qi  ', 'w   ', & !!! here, q means spec humidity
                'qr  ', 'qs  ', 'qg  ', 'qh  ' /)
...
do_getprs_tl     =lc_ps.and.lc_t .and.ls_prse             !!! do rh & do ps
do_normal_rh_to_q=lc_rh.and.lc_t .and.ls_prse.and.ls_q    !!! do rh
do_tv_to_tsen    =lc_t .and.ls_q .and.ls_tsen             !!! do rh & do tv
do_getuv         =lc_sf.and.lc_vp.and.ls_u.and.ls_v

!!! print *, 'cwy do_getprs_tl     =', do_getprs_tl
!!! print *, 'cwy do_normal_rh_to_q=', do_normal_rh_to_q, lc_rh, lc_t, ls_prse, ls_q
!!! print *, 'cwy do_tv_to_tsen    =', do_tv_to_tsen
!!! print *, 'cwy do_getuv         =', do_getuv
...
!!! create wbundel from xhat%step(jj)
...
!  Get pointers to required state variables
   call gsi_bundlegetpointer (sval(jj),'prse',sv_prse,istatus)  !!! prse=p3d in state_vector:: in anavinfo
   call gsi_bundlegetpointer (sval(jj),'tv'  ,sv_tv  ,istatus)  !!! tv   designated in anavinfo
   call gsi_bundlegetpointer (sval(jj),'tsen',sv_tsen,istatus)  !!! tsen designated in anavinfo
   call gsi_bundlegetpointer (sval(jj),'q'   ,sv_q   ,istatus)  !!! q state var, specific humidity
   call gsi_bundlegetpointer (wbundle ,'ps'  ,cv_ps  ,istatus)
   call gsi_bundlegetpointer (wbundle ,'t'   ,cv_t   ,istatus)  !!! q control var, virtual temperature
   call gsi_bundlegetpointer (wbundle ,'q'   ,cv_rh  ,istatus)  !!! q control var, relative humidity

!!! ----------------- sval%... = wbundle%...
!  Get 3d pressure
   if(do_getprs_tl) call getprs_tl(cv_ps,cv_t,sv_prse)  !!! getprs.f90, getprs TLM, out: sv_prse

!  Convert input normalized RH to q                     !!! q is specific humidity
   if(do_normal_rh_to_q) call normal_rh_to_q(cv_rh,cv_t,sv_prse,sv_q) !!! normal_rh_to_q.f90, out: sv_q

!  Calculate sensible temperature
   if(do_tv_to_tsen) call tv_to_tsen(cv_t,sv_q,sv_tsen) !!! tv_to_tsen.f90, out:sv_tsen

!  Copy other variables
   call gsi_bundlegetvar ( wbundle, 't'  , sv_tv,  istatus )  !!! out: sv_tv=wbundle%..%..
...
!!! ------------------------------------------------ chem
!!! pickup control var (cvars3d) in state var (sval) and save to control var (wbundle->xhat%step(jj))
!  Same one-to-one map for chemistry-vars; take care of them together 
   ...
   enddo
!           call prt_state_norms(sval(jj),'mysval')
!!! ------------------------------------------------ chem
