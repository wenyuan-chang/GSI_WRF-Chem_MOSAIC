...
! Check for alternative minimizations
  laltmin = lsqrtb.or.lbicg             !!! laltmin=F
...
!       Save information for next minimization
        ...
        elseif (l4dvar.or.lanczosave) then                  !!! l4dvar=F, lanczosave=F
           if (l4dvar) then
              clfile='obsdiags.ZZZ'
              write(clfile(10:12),'(I3.3)') jiter
              call obsdiags_write(clfile)  ! replacing write_obsdiags()
...
!       Save output of adjoint of analysis equation
        if (lobsensfc.or.iobsconv>0) call save_fc_sens      !!! lobsensfc=F, iobsconv=0
     endif
