...
!!!       stplimhowv,stplimcldch,stpjcdfi,stpjcpdry,stpliml
  use stpjcmod, only: stplimq,stplimg,stplimv,stplimp,stplimw10m,&
       stplimhowv,stplimcldch,stpjcdfi,stpjcpdry,stpliml,  stplimnum
!!! cwy mosaic ----------------------------------------------------- chem
...
!  integer(i_kind),parameter:: n0 = 12
  integer(i_kind),parameter:: n0 = 20
!!! cwy mosaic ----------------------------------------------------- chem
...
!!! cwy mosaic ----------------------------------------------------- chem
        if( .False. ) then
          call stplimnum(dval(ibin_anl),sval(ibin_anl),sges,pbc(1,13),pbc(1,14),nstep,ntguessig,'num_a01')
          if(ii == 1 )then
            pj(13,1)=pbc(1,13)+pbc(ipenloc,13)
            pj(14,1)=pbc(1,14)+pbc(ipenloc,14)
          end if
          call stplimnum(dval(ibin_anl),sval(ibin_anl),sges,pbc(1,15),pbc(1,16),nstep,ntguessig,'num_a02')
          if(ii == 1 )then
            pj(15,1)=pbc(1,15)+pbc(ipenloc,15)
            pj(16,1)=pbc(1,16)+pbc(ipenloc,16)
          end if
          call stplimnum(dval(ibin_anl),sval(ibin_anl),sges,pbc(1,17),pbc(1,18),nstep,ntguessig,'num_a03')
          if(ii == 1 )then
            pj(17,1)=pbc(1,17)+pbc(ipenloc,17)
            pj(18,1)=pbc(1,18)+pbc(ipenloc,18)
          end if
          call stplimnum(dval(ibin_anl),sval(ibin_anl),sges,pbc(1,19),pbc(1,20),nstep,ntguessig,'num_a04')
          if(ii == 1 )then
            pj(19,1)=pbc(1,19)+pbc(ipenloc,19)
            pj(20,1)=pbc(1,20)+pbc(ipenloc,20)
          end if
        end if
!!! cwy mosaic ----------------------------------------------------- chem
     end if
