subroutine read_aerosol_ce318(nread,ndata,nodata,infile,obstype,gstime,lunout, &
           twind,sis,nobs)
!
...
! Set constants.  Initialize variables
  ...
  if ( obstype == 'ce318_aod' ) then
     open(lunin,file=trim(infile),form='unformatted')
!!! cwy ------------------------------------------------- specified table
     open(99,file=trim(infile)//'.table')
     print *, 'cwy ce318burfr.table=', trim(infile)//'.table'
!!! cwy ------------------------------------------------- specified table
     ...
     if ( iret == 0 ) then
        if (subset == 'NC008041') then
           write(6,*)'READ_AEROSOL_CE318: CE318 data type, subset = ',subset
           nreal=6
           nchanl=20
           ...
           write(6,'(''READ_AEROSOL_CE318: aerosol bufr file '',a,''  date is '',i4,4i2.2,a)')trim(infile),iy,im,idd,ihh
...
!!! cwy ----------------------------------------------- wavnum
           ...
           write(6,*)'READ_AEROSOL_CE318:  *** WARNING: unknown aerosol data type, subset=',subset
           write(6,*)' infile=',infile, ', lunin=',lunin, ', obstype=',obstype
           write(6,*)' SKIP PROCESSING OF THIS MODIS FILE'
        endif

     else          ! read subset iret /= 0
        write(6,*)'READ_AEROSOL_CE318:  *** WARNING: read subset error, obstype=',obstype,', iret=',iret
     end if
     call closbf(lunin)
     close(lunin)
  else             ! obstype /= 'modis'
     write(6,*)'READ_AEROSOL_CE318:  *** WARNING: unknown aerosol input type, obstype=',obstype
  endif

  ! Deallocate satthin arrays
  call destroygrids

end subroutine read_aerosol_ce318
