...
!!! cwy mosaic ----------------------------------- chem
  use berror, only: alsize, dssize
  use obsmod, only: corr_length, nsize_mosaic_wrf
!!! cwy mosaic ----------------------------------- chem
...
! Declare local variables
  ...
  real(r_kind),dimension(nsig):: rate,dlsig,rlsig !!! should be rate(ndeg)?
...
!!! cwy mosaic ----------------------
  real(r_kind),dimension(nsize_mosaic_wrf,ndeg):: al
  integer(i_kind) :: kloc, yy, xx, zz, ksiz
  character( len=10 ) :: cnam
  character( len=2 ) :: c2
  real(r_kind), dimension(nsize_mosaic_wrf) :: ss
  real(r_kind),allocatable,dimension(:,:,:) :: corz_grid
  logical, parameter      ::  use_grid_bec=.false.   ! for GISPROC=1 only?
!!! cwy mosaic ----------------------
...
!!! cwy ----------------- mosaic
  allocate ( corz_grid(1:lat2,1:lon2,1:nsig) )
!!! cwy ----------------- mosaic
...
!!! in: ndge, out: be(ndeg), rate(ndeg)
  call rfdpar1(be,rate,ndeg)
!!! in: ndge, be(ndeg), rate(ndeg), out: turn(ndeg,ndeg), samp
...
!!! ------------------------------------------------------------------ vertical
        !!! cwy mosaic --------------------------------------------- chem
        !if( use_grid_bec .and. (index(cvars(n),"oin")/=0 .or. index(cvars(n),"mas")/=0 .or. index(cvars(n),"bc")/=0 ) ) then
        if( use_grid_bec ) then

            kloc=index(cvars(n),"_")
            cnam=trim(cvars(n))
            corz_grid=1.e-10
            open(991,file="../COVC_GRID/"//trim(cnam(1:kloc-1))//"_cov.txt",status="old",action="read")
            print *, 'cwy read ../COVC_GRID/'//trim(cnam(1:kloc-1))//'_cov.txt'
            do j=2, lat2-1
            do i=2, lon2-1
            do k=2, nsig-1
              read(991,*) yy, xx, zz, (ss(ksiz),ksiz=1,nsize_mosaic_wrf)
              do ksiz=1, nsize_mosaic_wrf
                write(c2,'(i2.2)') ksiz
                if( index(cvars(n),"a"//c2)/=0 .and. ss(ksiz)/=0. ) corz_grid(yy,xx,zz)=ss(ksiz)
              end do
            end do
            end do
            end do
            ...
            !!! initializes and renormalizes vertical smoothing coef. out: dsv
            !!! the specific smoothing coef dsv according to corr length scale vz
            do j=llmin,llmax
               call smoothzo(vz(1,j,n),samp,rate,n,j,dsv(1,1,j))
            end do
            do j=1,lat2
              ...
              do k=1,nsig
                ...
                dssv(j,i,k,n)=dssv(j,i,k,n)*corz_grid(j,i,k)*as3d(n) !!! dsv
              enddo
            end do
          end do
        !!! cwy mosaic --------------------------------------------- chem end
          ...
          do j=llmin,llmax
             ...
             do k=1,nsig
             do i=1,lon2
                dsv(i,k,j)=dsv(i,k,j)*corz(j,k,n)*as3d(n)    !!! dsv
             end do
             end do
          end do
...
! sli in scale  unit (can add in sea-land mask)
     ...
     do i=1,nx
        do j=1,ny
           fact=one/(one+(one-sl(j,i))*bw) !!! sl=1, fact=1, bw=0.
...
!!! ------------------------------------------------------------------ size bin
!!! START, NEED bkgcov.f90
!!! ------------------------------------------------------------------ size bin
! rate(ndeg), out:al(4,ndeg)
if( .true. ) then
if( nnnn1o>0 ) then
  call rfdparv(corr_length,rate,al,nsize_mosaic_wrf,ndeg)
  dssize=samp                     ! dssize
  do n=1, ndeg
    do ksiz=1, nsize_mosaic_wrf
      alsize(n,ksiz)=al(ksiz,n)         ! alsize(ndeg,nsizebin)
    end do
  end do
end if
end if
