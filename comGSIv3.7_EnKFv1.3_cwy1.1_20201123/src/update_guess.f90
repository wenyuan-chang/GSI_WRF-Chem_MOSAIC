...
!$$$
  ...
  use chemmod, only: laeroana_gocart, laeroana_mosaic
  use constants, only: eps, half, psv_a, psv_b, psv_c, psv_d
  use guess_grids, only: ges_prsl, ges_prsi
  use aerosol_water_awa, only: aerosol_water
  use obsmod, only: nsize_mosaic_wrf, n_cvmas, n_cvaer, n_cvnum, rmin_value
    !!! cwy mosaic --------------------------------------------- chem
...
! Declare local variables
  ...
  !!! cwy mosaic --------------------------------------------- chem
  real(r_kind),pointer,dimension(:,:,:) :: ptr3dfld =>NULL()
  real(r_kind),dimension(lat2,lon2,nsig),target   :: h2o_a0x
  real(r_kind),dimension(lat2,lon2,nsig),target   :: so4_a0x, no3_a0x, clx_a0x
  real(r_kind),dimension(lat2,lon2,nsig),target   :: ocx_a0x, bcx_a0x, oin_a0x
  real(r_kind)  ::  rso4, rno3, rclx, rocx, rbcx, roin
  real(r_kind)  ::  rl_hm, rdensity
  real(r_kind)  ::  tmp_K, fact, pw, prs_a, prs_v, prs_sv
  real(r_kind),parameter      ::  mw_so4=96., mw_no3=62., mw_clx=35.5
  ...
  real(r_kind),dimension(lat2,lon2,nsig) :: mas_a0x
  !!! cwy mosaic --------------------------------------------- chem
...
!    NOTE 2: the following assumes tv has same name in met-guess and increment vectors
     ...
     if (id>0) then
        ...
        if (.not.twodvar_regional .or. .not.tsensible) then   !!! tsensibel=F
            !!! ---------------------------------
...
!!! cwy mosaic ------------------------------------------------ chem
    if( laeroana_mosaic ) then
      do ksz=1, nsize_mosaic_wrf
        ...
        ! update control var mas_a0x
        if( n_cvmas/=0 ) then
          id=getindex(svars3d,'mas_a'//c2)
          if( id>0 ) then
            call gsi_bundlegetpointer (sval(ii),                'mas_a'//c2,ptr3dinc,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'mas_a'//c2,ptr3dges,istatus)
            call upd_positive_fldr3_(ptr3dges,ptr3dinc,tgmin)

            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'mas_a'//c2,ptr3dfld,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'num_a'//c2,ptr3dges,istatus)
            if( ksz==1 ) ptr3dges=ptr3dfld*wt_num_a01
            if( ksz==2 ) ptr3dges=ptr3dfld*wt_num_a02
            if( ksz==3 ) ptr3dges=ptr3dfld*wt_num_a03
            if( ksz==4 ) ptr3dges=ptr3dfld*wt_num_a04
            if( ksz==5 ) ptr3dges=ptr3dfld*wt_num_a05
            if( ksz==6 ) ptr3dges=ptr3dfld*wt_num_a06
            if( ksz==7 ) ptr3dges=ptr3dfld*wt_num_a07
            if( ksz==8 ) ptr3dges=ptr3dfld*wt_num_a08
          end if
        end if

        ! update control var num_a0x
        if( n_cvnum/=0 ) then
          id=getindex(svars3d,'num_a'//c2)
          if( id>0 ) then
            call gsi_bundlegetpointer (sval(ii),                'num_a'//c2,ptr3dinc,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'num_a'//c2,ptr3dges,istatus)
            call upd_positive_fldr3_(ptr3dges,ptr3dinc,tgmin)

            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'num_a'//c2,ptr3dfld,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'mas_a'//c2,ptr3dges,istatus)
            do k=1, nsig
            do j=1, lon2
            do i=1, lat2
              if( ksz==1 .and. wt_num_a01(i,j,k)>rmin_value ) ptr3dges(i,j,k)=ptr3dfld(i,j,k)/wt_num_a01(i,j,k)
              if( ksz==2 .and. wt_num_a02(i,j,k)>rmin_value ) ptr3dges(i,j,k)=ptr3dfld(i,j,k)/wt_num_a02(i,j,k)
              if( ksz==3 .and. wt_num_a03(i,j,k)>rmin_value ) ptr3dges(i,j,k)=ptr3dfld(i,j,k)/wt_num_a03(i,j,k)
              if( ksz==4 .and. wt_num_a04(i,j,k)>rmin_value ) ptr3dges(i,j,k)=ptr3dfld(i,j,k)/wt_num_a04(i,j,k)
              if( ksz==5 .and. wt_num_a05(i,j,k)>rmin_value ) ptr3dges(i,j,k)=ptr3dfld(i,j,k)/wt_num_a05(i,j,k)
              if( ksz==6 .and. wt_num_a06(i,j,k)>rmin_value ) ptr3dges(i,j,k)=ptr3dfld(i,j,k)/wt_num_a06(i,j,k)
              if( ksz==7 .and. wt_num_a07(i,j,k)>rmin_value ) ptr3dges(i,j,k)=ptr3dfld(i,j,k)/wt_num_a07(i,j,k)
              if( ksz==8 .and. wt_num_a08(i,j,k)>rmin_value ) ptr3dges(i,j,k)=ptr3dfld(i,j,k)/wt_num_a08(i,j,k)
            end do
            end do
            end do
          end if
        ...
        if( n_cvmas/=0 .or. n_cvnum/=0 ) then
          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'mas_a'//c2,ptr3dfld,istatus)
          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'so4_a'//c2,ptr3dges,istatus)
          if( ksz==1 ) ptr3dges=ptr3dfld*wt_so4_a01
          if( ksz==2 ) ptr3dges=ptr3dfld*wt_so4_a02
          if( ksz==3 ) ptr3dges=ptr3dfld*wt_so4_a03
          if( ksz==4 ) ptr3dges=ptr3dfld*wt_so4_a04
          if( ksz==5 ) ptr3dges=ptr3dfld*wt_so4_a05
          if( ksz==6 ) ptr3dges=ptr3dfld*wt_so4_a06
          if( ksz==7 ) ptr3dges=ptr3dfld*wt_so4_a07
          if( ksz==8 ) ptr3dges=ptr3dfld*wt_so4_a08

          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'mas_a'//c2,ptr3dfld,istatus)
          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'no3_a'//c2,ptr3dges,istatus)
          if( ksz==1 ) ptr3dges=ptr3dfld*wt_no3_a01
          if( ksz==2 ) ptr3dges=ptr3dfld*wt_no3_a02
          if( ksz==3 ) ptr3dges=ptr3dfld*wt_no3_a03
          if( ksz==4 ) ptr3dges=ptr3dfld*wt_no3_a04
          if( ksz==5 ) ptr3dges=ptr3dfld*wt_no3_a05
          if( ksz==6 ) ptr3dges=ptr3dfld*wt_no3_a06
          if( ksz==7 ) ptr3dges=ptr3dfld*wt_no3_a07
          if( ksz==8 ) ptr3dges=ptr3dfld*wt_no3_a08

          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'mas_a'//c2,ptr3dfld,istatus)
          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'nh4_a'//c2,ptr3dges,istatus)
          if( ksz==1 ) ptr3dges=ptr3dfld*wt_nh4_a01
          if( ksz==2 ) ptr3dges=ptr3dfld*wt_nh4_a02
          if( ksz==3 ) ptr3dges=ptr3dfld*wt_nh4_a03
          if( ksz==4 ) ptr3dges=ptr3dfld*wt_nh4_a04
          if( ksz==5 ) ptr3dges=ptr3dfld*wt_nh4_a05
          if( ksz==6 ) ptr3dges=ptr3dfld*wt_nh4_a06
          if( ksz==7 ) ptr3dges=ptr3dfld*wt_nh4_a07
          if( ksz==8 ) ptr3dges=ptr3dfld*wt_nh4_a08

          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'mas_a'//c2,ptr3dfld,istatus)
          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'oc_a'//c2,ptr3dges,istatus)
          if( ksz==1 ) ptr3dges=ptr3dfld*wt_ocx_a01
          if( ksz==2 ) ptr3dges=ptr3dfld*wt_ocx_a02
          if( ksz==3 ) ptr3dges=ptr3dfld*wt_ocx_a03
          if( ksz==4 ) ptr3dges=ptr3dfld*wt_ocx_a04
          if( ksz==5 ) ptr3dges=ptr3dfld*wt_ocx_a05
          if( ksz==6 ) ptr3dges=ptr3dfld*wt_ocx_a06
          if( ksz==7 ) ptr3dges=ptr3dfld*wt_ocx_a07
          if( ksz==8 ) ptr3dges=ptr3dfld*wt_ocx_a08

          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'mas_a'//c2,ptr3dfld,istatus)
          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'bc_a'//c2,ptr3dges,istatus)
          if( ksz==1 ) ptr3dges=ptr3dfld*wt_bcx_a01
          if( ksz==2 ) ptr3dges=ptr3dfld*wt_bcx_a02
          if( ksz==3 ) ptr3dges=ptr3dfld*wt_bcx_a03
          if( ksz==4 ) ptr3dges=ptr3dfld*wt_bcx_a04
          if( ksz==5 ) ptr3dges=ptr3dfld*wt_bcx_a05
          if( ksz==6 ) ptr3dges=ptr3dfld*wt_bcx_a06
          if( ksz==7 ) ptr3dges=ptr3dfld*wt_bcx_a07
          if( ksz==8 ) ptr3dges=ptr3dfld*wt_bcx_a08

          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'mas_a'//c2,ptr3dfld,istatus)
          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'oin_a'//c2,ptr3dges,istatus)
          if( ksz==1 ) ptr3dges=ptr3dfld*wt_oin_a01
          if( ksz==2 ) ptr3dges=ptr3dfld*wt_oin_a02
          if( ksz==3 ) ptr3dges=ptr3dfld*wt_oin_a03
          if( ksz==4 ) ptr3dges=ptr3dfld*wt_oin_a04
          if( ksz==5 ) ptr3dges=ptr3dfld*wt_oin_a05
          if( ksz==6 ) ptr3dges=ptr3dfld*wt_oin_a06
          if( ksz==7 ) ptr3dges=ptr3dfld*wt_oin_a07
          if( ksz==8 ) ptr3dges=ptr3dfld*wt_oin_a08

          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'mas_a'//c2,ptr3dfld,istatus)
          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'cl_a'//c2,ptr3dges,istatus)
          if( ksz==1 ) ptr3dges=ptr3dfld*wt_clx_a01
          if( ksz==2 ) ptr3dges=ptr3dfld*wt_clx_a02
          if( ksz==3 ) ptr3dges=ptr3dfld*wt_clx_a03
          if( ksz==4 ) ptr3dges=ptr3dfld*wt_clx_a04
          if( ksz==5 ) ptr3dges=ptr3dfld*wt_clx_a05
          if( ksz==6 ) ptr3dges=ptr3dfld*wt_clx_a06
          if( ksz==7 ) ptr3dges=ptr3dfld*wt_clx_a07
          if( ksz==8 ) ptr3dges=ptr3dfld*wt_clx_a08

          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'mas_a'//c2,ptr3dfld,istatus)
          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'na_a'//c2,ptr3dges,istatus)
          if( ksz==1 ) ptr3dges=ptr3dfld*wt_nax_a01
          if( ksz==2 ) ptr3dges=ptr3dfld*wt_nax_a02
          if( ksz==3 ) ptr3dges=ptr3dfld*wt_nax_a03
          if( ksz==4 ) ptr3dges=ptr3dfld*wt_nax_a04
          if( ksz==5 ) ptr3dges=ptr3dfld*wt_nax_a05
          if( ksz==6 ) ptr3dges=ptr3dfld*wt_nax_a06
          if( ksz==7 ) ptr3dges=ptr3dfld*wt_nax_a07
          if( ksz==8 ) ptr3dges=ptr3dfld*wt_nax_a08
        end if

        ! update control var aer_a0x
        if( n_cvaer/=0 ) then
          id=getindex(svars3d,'so4_a'//c2)
          if( id>0 ) then
            call gsi_bundlegetpointer (sval(ii),                'so4_a'//c2,ptr3dinc,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'so4_a'//c2,ptr3dges,istatus)
            call upd_positive_fldr3_(ptr3dges,ptr3dinc,tgmin)
          end if
          id=getindex(svars3d,'no3_a'//c2)
          if( id>0 ) then
            call gsi_bundlegetpointer (sval(ii),                'no3_a'//c2,ptr3dinc,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'no3_a'//c2,ptr3dges,istatus)
            call upd_positive_fldr3_(ptr3dges,ptr3dinc,tgmin)
          end if
          id=getindex(svars3d,'nh4_a'//c2)
          if( id>0 ) then
            call gsi_bundlegetpointer (sval(ii),                'nh4_a'//c2,ptr3dinc,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'nh4_a'//c2,ptr3dges,istatus)
            call upd_positive_fldr3_(ptr3dges,ptr3dinc,tgmin)
          end if
          id=getindex(svars3d,'oc_a'//c2)
          if( id>0 ) then
            call gsi_bundlegetpointer (sval(ii),                'oc_a'//c2,ptr3dinc,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'oc_a'//c2,ptr3dges,istatus)
            call upd_positive_fldr3_(ptr3dges,ptr3dinc,tgmin)
          end if
          id=getindex(svars3d,'bc_a'//c2)
          if( id>0 ) then
            call gsi_bundlegetpointer (sval(ii),                'bc_a'//c2,ptr3dinc,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'bc_a'//c2,ptr3dges,istatus)
            call upd_positive_fldr3_(ptr3dges,ptr3dinc,tgmin)
          end if
          id=getindex(svars3d,'oin_a'//c2)
          if( id>0 ) then
            call gsi_bundlegetpointer (sval(ii),                'oin_a'//c2,ptr3dinc,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'oin_a'//c2,ptr3dges,istatus)
            call upd_positive_fldr3_(ptr3dges,ptr3dinc,tgmin)
          end if
          id=getindex(svars3d,'cl_a'//c2)
          if( id>0 ) then
            call gsi_bundlegetpointer (sval(ii),                'cl_a'//c2,ptr3dinc,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'cl_a'//c2,ptr3dges,istatus)
            call upd_positive_fldr3_(ptr3dges,ptr3dinc,tgmin)
          end if
          id=getindex(svars3d,'na_a'//c2)
          if( id>0 ) then
            call gsi_bundlegetpointer (sval(ii),                'na_a'//c2,ptr3dinc,istatus)
            call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'na_a'//c2,ptr3dges,istatus)
            call upd_positive_fldr3_(ptr3dges,ptr3dinc,tgmin)
          end if
          ...
          call gsi_bundlegetpointer (gsi_chemguess_bundle(it),'num_a'//c2,ptr3dges,istatus)
          if( ksz==1 ) ptr3dges=mas_a0x*wt_num_a01
          if( ksz==2 ) ptr3dges=mas_a0x*wt_num_a02
          if( ksz==3 ) ptr3dges=mas_a0x*wt_num_a03
          if( ksz==4 ) ptr3dges=mas_a0x*wt_num_a04
          if( ksz==5 ) ptr3dges=mas_a0x*wt_num_a05
          if( ksz==6 ) ptr3dges=mas_a0x*wt_num_a06
          if( ksz==7 ) ptr3dges=mas_a0x*wt_num_a07
          if( ksz==8 ) ptr3dges=mas_a0x*wt_num_a08
        end if
      end do

      !!! --------------------------- awa ---------------------------------
      ! update state var water_a0x
      do ksz=1, nsize_mosaic_wrf
        write(c2,'(i2.2)') ksz
        ...
        do j=1, lon2
        do i=1, lat2
          ! humidity, fractional unit, 0-1
          if( k==1 ) then
            fact     =one+fv*ges_q(i,j,k)
            pw       =eps+ges_q(i,j,k)*(one-eps)
            tmp_K    =ges_tv(i,j,k)/fact
            prs_sv   =exp(psv_a*tmp_K**2+psv_b*tmp_K+psv_c+psv_d/tmp_K) ! Pa
            prs_a    =1000.0_r_kind*exp( half*(log(ges_prsi(i,j,k,it))+log(ges_prsl(i,j,k,it))) ) ! Pa
            prs_v    =ges_q(i,j,k)*prs_a/pw ! vapor pressure, Pa
            rdensity =exp( half*log(ges_prsi(i,j,k,it))+log(ges_prsl(i,j,k,it)) )/(ges_tv(i,j,k)*287.058) ! air density, kg/m3
          else
            ...
            prs_sv   =exp(psv_a*tmp_K**2+psv_b*tmp_K+psv_c+psv_d/tmp_K) ! Pa
            prs_a    =1000.0_r_kind*exp( half*(log(ges_prsl(i,j,k-1,it))+log(ges_prsl(i,j,k,it))) ) ! Pa
            prs_v    =half*(ges_q(i,j,k-1)+ges_q(i,j,k))*prs_a/pw ! vapor pressure, Pa
            rdensity =exp( half*log(ges_prsl(i,j,k-1,it))+log(ges_prsl(i,j,k,it)) )/(ges_tv(i,j,k)*287.058) ! air density, kg/m3
          end if
          rl_hm      =prs_v/prs_sv ! rhum, 0-1
          ! ug/kg -> nmol/m3
          rso4=so4_a0x(i,j,k)*rdensity/mw_so4*1.e+3
          rno3=no3_a0x(i,j,k)*rdensity/mw_no3*1.e+3
          rclx=clx_a0x(i,j,k)*rdensity/mw_clx*1.e+3
          ! ug/kg -> ng/m3
          rocx=ocx_a0x(i,j,k)*rdensity*1.e+3
          rbcx=bcx_a0x(i,j,k)*rdensity*1.e+3
          roin=oin_a0x(i,j,k)*rdensity*1.e+3
          ! mas_oax, kg(aerosol_water)/m3(air)
          h2o_a0x(i,j,k)=aerosol_water( rso4, rno3, rclx, rocx, rbcx, roin, rl_hm )
          h2o_a0x(i,j,k)=h2o_a0x(i,j,k)/rdensity*1.e+9
        end do
        end do
        end do
