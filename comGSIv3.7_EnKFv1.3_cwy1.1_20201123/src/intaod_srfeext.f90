...
!$$$ end documentation block
  ...
  private
  public :: intaod_srfeext

  interface intaod_srfeext; module procedure &
            intaod_srfeext_
  end interface

contains

  subroutine intaod_srfeext_(srfeexthead,rval,sval)
...
!    Distribute adjoint contributions over surrounding grid points
          ...
          do ic = 1, naero
             ...
             if ( istatus /= 0 ) then
                write(6,*) 'error gsi_bundlegetpointer in intaod_srfeext for ', aerojacnames(ic)
             end if
             do k=1,nsig
                n = k + nsig*(ic-1)
                i1 = i1n(k)
...
!       call stop2(999)
    ...
    return
  end subroutine intaod_srfeext_

end module intaodmod_srfeext
