...
!!! cwy srf_eabs_esca --------------------------------
       case(i_srfeabs_ob_type)
          call stpaod_srfeabs(yobs(ib)%srfeabs,dval(ib),xval(ib),pbcjo(1,i_srfeabs_ob_type,ib),sges,nstep)
       case(i_srfesca_ob_type)
          call stpaod_srfesca(yobs(ib)%srfesca,dval(ib),xval(ib),pbcjo(1,i_srfesca_ob_type,ib),sges,nstep)
       case(i_srfeext_ob_type)
          call stpaod_srfeext(yobs(ib)%srfeext,dval(ib),xval(ib),pbcjo(1,i_srfeext_ob_type,ib),sges,nstep)
!!! cwy srf_eabs_esca --------------------------------
...
!!! cwy srf_eabs_esca ----------------------------------
       case(i_srfeabs_ob_type)
!         penalty, b, and c for srfeabs
          if(associated(yobs(ib)%srfeabs)) then
             stpcnt = stpcnt +1
             ll_jo(stpcnt) = i_srfeabs_ob_type
             ib_jo(stpcnt) = ib
          end if
       case(i_srfesca_ob_type)
!         penalty, b, and c for srfesca
          if(associated(yobs(ib)%srfesca)) then
             stpcnt = stpcnt +1
             ll_jo(stpcnt) = i_srfesca_ob_type
             ib_jo(stpcnt) = ib
          end if
       case(i_srfeext_ob_type)
!         penalty, b, and c for srfeext
          if(associated(yobs(ib)%srfeext)) then
             stpcnt = stpcnt +1
             ll_jo(stpcnt) = i_srfeext_ob_type
             ib_jo(stpcnt) = ib
          end if
!!! cwy srf_eabs_esca ----------------------------------
       case(i_pm10_ob_type)
