...
!$$$ end documentation block
  ...
  use m_srfeabsNode , only: srfeabsNode
  use m_srfeabsNode , only: srfeabsNode_typecast
  use m_srfeabsNode , only: srfeabsNode_nextcast
  implicit none

  private
  public :: intaod_srfeabs

  interface intaod_srfeabs; module procedure &
            intaod_srfeabs_
  end interface

contains

  subroutine intaod_srfeabs_(srfeabshead,rval,sval)
...
! Declare passed variables
    class(obsNode),pointer,intent(in) :: srfeabshead
...
!   real(r_kind) cg_aero,p0,wnotgross,wgross
    type(srfeabsNode), pointer :: aeroptr
  ...
  ! Inquire about chemistry
    ...
    !aeroptr => aerohead
    aeroptr => srfeabsNode_typecast(srfeabshead)
...
!    Distribute adjoint contributions over surrounding grid points
          ...
          do ic = 1, naero
             ...
             if ( istatus /= 0 ) then
                write(6,*) 'error gsi_bundlegetpointer in intaod_srfeabs for ', aerojacnames(ic)
             end if
             do k=1,nsig
                n = k + nsig*(ic-1)
                i1 = i1n(k)
       ...
       aeroptr => srfeabsNode_nextcast(aeroptr)
!       call stop2(999)
    end do

    return
  end subroutine intaod_srfeabs_

end module intaodmod_srfeabs
