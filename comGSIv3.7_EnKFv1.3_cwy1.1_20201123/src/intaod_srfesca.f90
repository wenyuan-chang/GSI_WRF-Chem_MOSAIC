...
!$$$ end documentation block
  ...
  use m_srfescaNode , only: srfescaNode
  use m_srfescaNode , only: srfescaNode_typecast
  use m_srfescaNode , only: srfescaNode_nextcast
  implicit none

  private
  public :: intaod_srfesca

  interface intaod_srfesca; module procedure &
            intaod_srfesca_
  end interface

contains

  subroutine intaod_srfesca_(srfescahead,rval,sval)
...
! Declare passed variables
    class(obsNode),pointer,intent(in) :: srfescahead
...
!   real(r_kind) cg_aero,p0,wnotgross,wgross
    type(srfescaNode), pointer :: aeroptr
  ...
  ! Inquire about chemistry
    ...
    !aeroptr => aerohead
    aeroptr => srfescaNode_typecast(srfescahead)
...
!    Distribute adjoint contributions over surrounding grid points
          ...
          do ic = 1, naero
             ...
             if ( istatus /= 0 ) then
                write(6,*) 'error gsi_bundlegetpointer in intaod_srfesca for ', aerojacnames(ic)
             end if
             do k=1,nsig
                n = k + nsig*(ic-1)
                i1 = i1n(k)
       ...
       aeroptr => srfescaNode_nextcast(aeroptr)
!       call stop2(999)
    end do

    return
  end subroutine intaod_srfesca_

end module intaodmod_srfesca
