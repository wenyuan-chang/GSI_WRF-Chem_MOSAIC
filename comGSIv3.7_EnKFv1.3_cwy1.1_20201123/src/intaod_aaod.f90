...
!$$$ end documentation block
  ...
  interface intaod_aaod; module procedure &
            intaod_aaod_
  end interface

contains

  subroutine intaod_aaod_(aaodhead,rval,sval)
...
!    Distribute adjoint contributions over surrounding grid points
          ...
          do ic = 1, naero
             ...
             if ( istatus /= 0 ) then
                write(6,*) 'error gsi_bundlegetpointer in intaod_aaod for ', aerojacnames(ic)
             end if
             do k=1,nsig
                n = k + nsig*(ic-1)
                i1 = i1n(k)
...
!       call stop2(999)
    ...
    return
  end subroutine intaod_aaod_

end module intaodmod_aaod
