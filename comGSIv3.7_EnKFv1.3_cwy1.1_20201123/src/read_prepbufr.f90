...
!  Set qc limits based on noiqc flag
  ...
  if(perturb_obs .and. (tob .or. psob .or. qob))nreal=nreal+1 !!! perturb_obs=F
  ...
  if (blacklst) call blacklist_read(obstype)  !!! blacklst=F
...
!------------------------------------------------------------------------
  ...
  !!! ------------------------------------------------
  do nc=1,nconvtype !!! nconvtype= the number of obs lines in convinfo, convinfo.f90
     if(trim(ioctype(nc)) == trim(obstype))then !!! ioctype=otype, ictype=type in convinfo, convinfo.f90, use_prepb_satwnd=F
       if(.not.use_prepb_satwnd .and. (trim(ioctype(nc)) == 'uv' .or. trim(ioctype(nc)) == 'wspd10m' .or. & 
          ...
          if (aircraft_t_bc) then !!! aircraft_t_bc=F
     ...
     end if  !!! icuse=iuse in convinfo, convinf.f90
     if(trim(ioctype(nc)) == trim(obstype) .and. abs(icuse(nc)) <= 1)then
        if(.not.use_prepb_satwnd .and. (trim(ioctype(nc)) == 'uv' .or. trim(ioctype(nc)) == 'wspd10m' .or. &
          ...
          !!! ------------------
           ithin=ithin_conv(nc)  !!! ithin_conv=ithin in convinfo, convinfo.f90
           if(ithin > 0)then
              ntread=ntread+1
              ntx(ntread)=nc
           end if
...
!! get message and subset counts
  ...
  call getcount_bufr(infile,nmsgmax,mxtb)   !!! getcount_bufr.f90
  ...
  !!! -------------------------------------------------------------------
  !!!       PART 1: handle convinfo & bufr hdr2
  !!! -------------------------------------------------------------------
...
!    Time offset
     if(nmsg == 0) call time_4dvar(idate,toff)      !!! gsi_4dvar.f90, output: toff
     nmsg=nmsg+1
...
!       Extract type information                  !!! real(r_double),dimension(8):: hdr,hdrtsb
  !!! ...............................................
        call ufbint(lunin,hdr,4,1,iret,hdstr2)    !!! hdstr2=/'TYP SAID T29 SID'/
  !!!  TYP =PREPBUFR REPORT TYPE
  !!!  https://www.emc.ncep.noaa.gov/mmb/data_processing/prepbufr.doc/table_1.htm  search TYP
  !!!
  !!!  SAID=SATELLITE IDENTIFIER (TABLE ENTRIES "SATWND", "SATEMP", "GOESND" ...)
  !!!  https://www.emc.ncep.noaa.gov/mmb/data_processing/common_tbl_c1-c5.htm#c-5
  !!!
  !!!  T29 =DATA DUMP REPORT TYPE
  !!!  https://www.emc.ncep.noaa.gov/mmb/data_processing/prepbufr.doc/table_1.htm  search T29
  !!!
  !!!  SID =STATION IDENTIFICATION
  !!!  https://www.emc.ncep.noaa.gov/mmb/data_processing/prepbufr.doc/table_1.htm  search SID
  !!! ............................................... END
        kx=hdr(1)     !!! hdr(1)="TYP"
        ...
        if(kx==224 .and. .not.newvad) then
           call ufbint(lunin,hdrtsb,1,1,iret,'TSB')   !!! TSB=REPORT SUBTYPE
           if(hdrtsb(1)==2) then
              newvad=.true.
           else
              call ufbint(lunin,obsdat,13,255,levs,obstr)     !!! obstr=/'POB QOB TOB ZOB UOB VOB PWO MXGS HOVI CAT PRSS TDO PMO' /
              if(levs>1)then
                 do k=1, levs-1
...
!  (see https://www.wmo.int/pages/prog/amp/mmop/wmo-number-rules.html)  Set kx to 199/299
        if (id_drifter .and. (kx==180 .or. kx==280) .and. nint(hdr(3))==562) then !!! id_drifter=F
           rstation_id=hdr(4)
...
! temporary specify iobsub until put in bufr file
        iobsub = 0                                                  
        if(kx == 280 .or. kx == 180 ) iobsub=hdr(3)       !!! hdr(3)->"T29" 
...
!       if(kx == 289 .or. kx == 290) iobsub=hdr(2)
        ...
        if(kx == 290) iobsub=hdr(2)     !!! hdr(2)->"SAID"
...
!  Find convtype which match ob type and subtype group (isubtype == ?*) !       where ? specifies the group and icsubtype = ?0) ixsub=icsubtype(nc)/10
              iosub=iobsub/10
...
!  Save information for next read
        if(ncsave /= 0) then
  !!! ...............................................
           call ufbint(lunin,levdat,1,255,levs,levstr)   !!! levstr=/'POB'/, PRESSURE OBSERVATION, Units: MB
  !!! ............................................... END
           maxobs=maxobs+max(1,levs)
           nx=1
           if(ithin_conv(ncsave) > 0)then     !!! ithin_conv in convinfo(.f90), =0 no thinning
              do ii=2,ntread
                 if(ntx(ii) == ncsave)nx=ii
              end do
           end if
           tab(ntb,1)=ncsave      !!! tab(mxtb,3)
           tab(ntb,2)=nx
           tab(ntb,3)=levs
           lmsg(nmsg,nx) = .true.
        end if
  ...
  enddo msg_report
  !!! -------------------------------------------------------------------
  !!!       PART 1: handle convinfo & bufr hdr2, END
  !!! -------------------------------------------------------------------
...
! Obtain program code (VTCD) associated with "VIRTMP" step
  call ufbqcd(lunin,'VIRTMP',vtcd)                          !!! flag to mark whether t is tv
!!!  print *, ' vtcd=', vtcd
...
!Obtain code if it exists.  Otherwise set to missing (-999)
  ...
  call init_rjlists           !!! sfcobsqc.f90 -> call readin_rjlists(...) read the reject lists
  call init_aircraft_rjlists
  if(i_gsdsfc_uselist==1) call init_gsd_sfcuselist  !!! i_gsdsfc_uselist=0

  if (lhilbert) call init_hilbertcurve(maxobs)      !!! lhilbert=F
...
! loop over convinfo file entries; operate on matches
  ...
  !!!! ------------------------------------------------------------------------
  loop_convinfo: do nx=1, ntread    !!! ntread, the number of obs type that needs thinning
  !!!! ------------------------------------------------------------------------
...
!          Extract type, date, and location information
  !!! ...............................................
           call ufbint(lunin,hdr,8,1,iret,hdstr)        !!! data hdstr  /'SID XOB YOB DHR TYP ELV SAID T29'/
  !!! ...............................................
           kx=hdr(5)    !!! TYP
...
!------------------------------------------------------------------------
           ...
           if(offtime_data) then    !!! offtime_data=T, obsmod.F90, =T to allow use of obs files with ref time different from analysis time
  !!! ...............................................
...
!          Balloon drift information available for these data
           ...
           if (.not. (aircraft_t_bc .and. acft_profl_file)) then
              if (l4dvar.or.l4densvar) then   !!! l4dvar=F, l4densvar=F
                 if ((t4dv<zero.OR.t4dv>winlen) .and. .not.driftl) cycle loop_readsb ! outside time window
              else      !!! ctwind=twindow which is in convinfo
...
!          skip this observation.  This check must be done before thinning.
  ...
  !!! ...............................................
           ...
           if (sfctype) then
              call ufbint(lunin,r_prvstg,1,1,iret,prvstr)     !!! data prvstr /'PRVSTG'/   
              call ufbint(lunin,r_sprvstg,1,1,iret,sprvstr)   !!! data sprvstr /'SPRVSTG'/ 
  !!! ............................................... END
           else
              c_prvstg=cspval
              c_sprvstg=cspval
...
!          Extract data information on levels
  !!! ...............................................
           call ufbint(lunin,obsdat,13,255,levs,obstr)        !!! obstr data obstr  /'POB QOB TOB ZOB UOB VOB PWO MXGS HOVI CAT PRSS TDO PMO' /
           if (twodvar_regional) then
           ...
           call ufbint(lunin,qcmark,8,255,levs,qcstr) !!! qcstr=/'PQM QQM TQM ZQM WQM NUL PWQ PMQ'/
           call ufbint(lunin,obserr,8,255,levs,oestr) !!! oestr=/'POE QOE TOE NUL WOE NUL PWE     '/
           call ufbevn(lunin,tpc,1,255,20,levs,'TPC')
  !!! ............................................... END

!          If available, get obs errors from error table
           
           if(oberrflg .and. kx<= 300)then      !!! oberrflg=T
...
!                        write(6,*) 'READ_PREPBUFR_Pw:',itypex,itypey,ierr_pw,k2_pw,ierr_pw,nc,kx,ppb
  ...
  !!! ............................................... 
                do k=1,levs
                   itypex=kx
                   ppb=obsdat(1,k)                            !!! obsdat( 1,k)= 'POB', POB=PRSE in my bufr
                   if(kx==153)ppb=obsdat(11,k)*0.01_r_kind    !!! obsdat(11,k)= 'PRSS', kx=181 in my bufr
                   ppb=max(zero,min(ppb,r2000))
                   ...
                   obserr(3,k)=(one-del)*etabl(itypex,k1,2)+del*etabl(itypex,k2,2)  !!! T
                   obserr(2,k)=(one-del)*etabl(itypex,k1,3)+del*etabl(itypex,k2,3)  !!! RH
                   obserr(5,k)=(one-del)*etabl(itypex,k1,4)+del*etabl(itypex,k2,4)  !!! UV
                   obserr(1,k)=(one-del)*etabl(itypex,k1,5)+del*etabl(itypex,k2,5)  !!! Ps
                   obserr(7,k)=(one-del)*etabl(itypex,k1,6)+del*etabl(itypex,k2,6)  !!! PW

                   obserr(3,k)=max(obserr(3,k),terrmin) !!! terrmin=0.5
                   obserr(2,k)=max(obserr(2,k),qerrmin) !!! qerrmin=0.05
                   obserr(5,k)=max(obserr(5,k),werrmin) !!! werrmin=1.
                   obserr(1,k)=max(obserr(1,k),perrmin) !!! perrmin=0.3
                   obserr(7,k)=max(obserr(7,k),pwerrmin)!!! pwerrmin=1.
                enddo
  !!! ............................................... END
              !!! ------------------------------------- njqc
...
!          If data with drift position, get drift information
           if(driftl) call ufbint(lunin,drfdat,8,255,iret,drift)   !!! data drift  /'XDR YDR HRDR                    '/
     
! raob level enhancement on temp and q obs    !!! ext_sonde=T
           !!! sonde_ext for kx=120, 220, ADPUDA, RAWINSONDE
...
!          versus sensible temperature
           if(tob) then
              if (.not. twodvar_regional .or. .not.tsensible) then !!! twodvar_regional=F, tsensible=F
                 !!! ...............................................
                 do k=1,levs
...
!             Set usage variable              
  ...
  !!! ............................................... site reject list?
                    call get_usagerj(kx,obstype,c_station_id,c_prvstg,c_sprvstg, &    !!! sfcobsqc.f90
                                            dlon_earth,dlat_earth,idate,t4dv-toff,      &
                                            obsdat(5,k),obsdat(6,k),usage)  !!! obsdat(5:6,k)=U, V
              !!! if usage>=6 do nothing
              !!! qm=0-2, usage=0; usage<=jiter -> muse=True in setupq.f90... obs data will be used in DA.
                 endif
  !!! ............................................... END
              endif
...
!!! ------------------------------------------------------------------------------------------
                 ...
                 !!! ------------------------------ errormod in qcmod.f90
                    call errormod(pqm,qqm,levs,plevs,errout,k,presl,dpres,nsig,lim_qm)
                 !!! ------------------------------
                 end if
                 !!! ------------------------------
                 qoe=obserr(2,k)*one_tenth*errout         !!! obserr(2,k)=0.39, errout=1
                 !!! ------------------------------
                 if (inflate_error) then
                    qmaxerr=emerr*r0_7; qoe=qoe*r1_2
                 end if                           !!! obsdat is specific humidity, shum
                 qobcon=obsdat(2,k)*convert       !!! convert=1.e-6; data*1.e+6 in the input prepbufr, qobcon is shum
                 tdry=r999
                 if (tqm(k)<lim_tqm) tdry=(obsdat(3,k)+t0c)/(one+fv*qobcon) !!!  fv=0.608 in constants.f90
