...
!   end if
  ...
  !$$$
    ...
    use obsmod, only: nsize_mosaic_wrf
    !!! cwy mosaic ------------------------------------- chem
    use gsi_bundlemod, only: GSI_BundleGetPointer
    use gsi_metguess_mod, only: gsi_metguess_get,GSI_MetGuess_Bundle
...
!    The 9 3D cloud analysis fields are: ql,qi,qr,qs,qg,qnr,qni,qnc,tt
    ...
    if ( laeroana_mosaic ) then
       call gsi_chemguess_get ( 'aerosols::3d', n_mosaic_var, ier )
  ...
  ! for hydrometeors
    ...
    if(mype == 0) then
       ...
       open (lendian_in,file=filename,form='unformatted')         !!! sigf03
       open (lendian_out,file='siganl',form='unformatted')        !!! siganl
  ...
  ! for hydrometeors
    ...
    if ( laeroana_mosaic ) then
       ...
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'so4_a01',  ges_so4_a01,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'so4_a02',  ges_so4_a02,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'so4_a03',  ges_so4_a03,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'so4_a04',  ges_so4_a04,  istatus );ier=ier+istatus 
       if( nsize_mosaic_wrf==8 ) then
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'so4_a05',  ges_so4_a05,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'so4_a06',  ges_so4_a06,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'so4_a07',  ges_so4_a07,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'so4_a08',  ges_so4_a08,  istatus );ier=ier+istatus 
       end if

       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'no3_a01',  ges_no3_a01,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'no3_a02',  ges_no3_a02,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'no3_a03',  ges_no3_a03,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'no3_a04',  ges_no3_a04,  istatus );ier=ier+istatus 
       if( nsize_mosaic_wrf==8 ) then
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'no3_a05',  ges_no3_a05,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'no3_a06',  ges_no3_a06,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'no3_a07',  ges_no3_a07,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'no3_a08',  ges_no3_a08,  istatus );ier=ier+istatus 
       end if

       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'nh4_a01',  ges_nh4_a01,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'nh4_a02',  ges_nh4_a02,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'nh4_a03',  ges_nh4_a03,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'nh4_a04',  ges_nh4_a04,  istatus );ier=ier+istatus 
       if( nsize_mosaic_wrf==8 ) then
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'nh4_a05',  ges_nh4_a05,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'nh4_a06',  ges_nh4_a06,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'nh4_a07',  ges_nh4_a07,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'nh4_a08',  ges_nh4_a08,  istatus );ier=ier+istatus 
       end if

       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oc_a01',  ges_ocx_a01,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oc_a02',  ges_ocx_a02,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oc_a03',  ges_ocx_a03,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oc_a04',  ges_ocx_a04,  istatus );ier=ier+istatus 
       if( nsize_mosaic_wrf==8 ) then
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oc_a05',  ges_ocx_a05,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oc_a06',  ges_ocx_a06,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oc_a07',  ges_ocx_a07,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oc_a08',  ges_ocx_a08,  istatus );ier=ier+istatus 
       end if

       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'bc_a01',  ges_bcx_a01,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'bc_a02',  ges_bcx_a02,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'bc_a03',  ges_bcx_a03,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'bc_a04',  ges_bcx_a04,  istatus );ier=ier+istatus 
       if( nsize_mosaic_wrf==8 ) then
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'bc_a05',  ges_bcx_a05,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'bc_a06',  ges_bcx_a06,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'bc_a07',  ges_bcx_a07,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'bc_a08',  ges_bcx_a08,  istatus );ier=ier+istatus 
       end if

       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'cl_a01',  ges_clx_a01,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'cl_a02',  ges_clx_a02,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'cl_a03',  ges_clx_a03,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'cl_a04',  ges_clx_a04,  istatus );ier=ier+istatus 
       if( nsize_mosaic_wrf==8 ) then
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'cl_a05',  ges_clx_a05,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'cl_a06',  ges_clx_a06,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'cl_a07',  ges_clx_a07,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'cl_a08',  ges_clx_a08,  istatus );ier=ier+istatus 
       end if

       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'na_a01',  ges_nax_a01,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'na_a02',  ges_nax_a02,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'na_a03',  ges_nax_a03,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'na_a04',  ges_nax_a04,  istatus );ier=ier+istatus 
       if( nsize_mosaic_wrf==8 ) then
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'na_a05',  ges_nax_a05,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'na_a06',  ges_nax_a06,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'na_a07',  ges_nax_a07,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'na_a08',  ges_nax_a08,  istatus );ier=ier+istatus 
       end if

       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oin_a01',  ges_oin_a01,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oin_a02',  ges_oin_a02,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oin_a03',  ges_oin_a03,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oin_a04',  ges_oin_a04,  istatus );ier=ier+istatus 
       if( nsize_mosaic_wrf==8 ) then
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oin_a05',  ges_oin_a05,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oin_a06',  ges_oin_a06,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oin_a07',  ges_oin_a07,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'oin_a08',  ges_oin_a08,  istatus );ier=ier+istatus 
       end if

       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'num_a01',  ges_num_a01,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'num_a02',  ges_num_a02,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'num_a03',  ges_num_a03,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'num_a04',  ges_num_a04,  istatus );ier=ier+istatus 
       if( nsize_mosaic_wrf==8 ) then
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'num_a05',  ges_num_a05,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'num_a06',  ges_num_a06,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'num_a07',  ges_num_a07,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'num_a08',  ges_num_a08,  istatus );ier=ier+istatus 
       end if

       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'water_a01',  ges_h2o_a01,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'water_a02',  ges_h2o_a02,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'water_a03',  ges_h2o_a03,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'water_a04',  ges_h2o_a04,  istatus );ier=ier+istatus 
       if( nsize_mosaic_wrf==8 ) then
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'water_a05',  ges_h2o_a05,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'water_a06',  ges_h2o_a06,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'water_a07',  ges_h2o_a07,  istatus );ier=ier+istatus 
       call GSI_BundleGetPointer ( GSI_ChemGuess_Bundle(it), 'water_a08',  ges_h2o_a08,  istatus );ier=ier+istatus 
       end if

       if (ier/=0 .and. mype == 0) then
           write(6,*)'WRWRFMASSA_NETCDF: getpointer failed for mosaic species'
       endif
       do iv = 1, n_mosaic_var
          kchem(iv) = i_chem(iv)-1
       end do
  ...
  ! for hydrometeors      
            ...
            !!! cwy mosaic ------------------------------------------------ chem
             if ( laeroana_mosaic ) then
                if( nsize_mosaic_wrf==4 ) then
                all_loc(j,i,kchem( 1))=ges_so4_a01(j,i,k)
                all_loc(j,i,kchem( 2))=ges_so4_a02(j,i,k)
                all_loc(j,i,kchem( 3))=ges_so4_a03(j,i,k)
                all_loc(j,i,kchem( 4))=ges_so4_a04(j,i,k)

                all_loc(j,i,kchem( 5))=ges_no3_a01(j,i,k)
                all_loc(j,i,kchem( 6))=ges_no3_a02(j,i,k)
                all_loc(j,i,kchem( 7))=ges_no3_a03(j,i,k)
                all_loc(j,i,kchem( 8))=ges_no3_a04(j,i,k)

                all_loc(j,i,kchem( 9))=ges_nh4_a01(j,i,k)
                all_loc(j,i,kchem(10))=ges_nh4_a02(j,i,k)
                all_loc(j,i,kchem(11))=ges_nh4_a03(j,i,k)
                all_loc(j,i,kchem(12))=ges_nh4_a04(j,i,k)

                all_loc(j,i,kchem(13))=ges_ocx_a01(j,i,k)
                all_loc(j,i,kchem(14))=ges_ocx_a02(j,i,k)
                all_loc(j,i,kchem(15))=ges_ocx_a03(j,i,k)
                all_loc(j,i,kchem(16))=ges_ocx_a04(j,i,k)

                all_loc(j,i,kchem(17))=ges_bcx_a01(j,i,k)
                all_loc(j,i,kchem(18))=ges_bcx_a02(j,i,k)
                all_loc(j,i,kchem(19))=ges_bcx_a03(j,i,k)
                all_loc(j,i,kchem(20))=ges_bcx_a04(j,i,k)

                all_loc(j,i,kchem(21))=ges_clx_a01(j,i,k)
                all_loc(j,i,kchem(22))=ges_clx_a02(j,i,k)
                all_loc(j,i,kchem(23))=ges_clx_a03(j,i,k)
                all_loc(j,i,kchem(24))=ges_clx_a04(j,i,k)

                all_loc(j,i,kchem(25))=ges_nax_a01(j,i,k)
                all_loc(j,i,kchem(26))=ges_nax_a02(j,i,k)
                all_loc(j,i,kchem(27))=ges_nax_a03(j,i,k)
                all_loc(j,i,kchem(28))=ges_nax_a04(j,i,k)

                all_loc(j,i,kchem(29))=ges_oin_a01(j,i,k)
                all_loc(j,i,kchem(30))=ges_oin_a02(j,i,k)
                all_loc(j,i,kchem(31))=ges_oin_a03(j,i,k)
                all_loc(j,i,kchem(32))=ges_oin_a04(j,i,k)

                all_loc(j,i,kchem(33))=ges_num_a01(j,i,k)
                all_loc(j,i,kchem(34))=ges_num_a02(j,i,k)
                all_loc(j,i,kchem(35))=ges_num_a03(j,i,k)
                all_loc(j,i,kchem(36))=ges_num_a04(j,i,k)

                all_loc(j,i,kchem(37))=ges_h2o_a01(j,i,k)
                all_loc(j,i,kchem(38))=ges_h2o_a02(j,i,k)
                all_loc(j,i,kchem(39))=ges_h2o_a03(j,i,k)
                all_loc(j,i,kchem(40))=ges_h2o_a04(j,i,k)
                end if
                ...
                all_loc(j,i,kchem( 3))=ges_so4_a03(j,i,k)
                all_loc(j,i,kchem( 4))=ges_so4_a04(j,i,k)
                all_loc(j,i,kchem( 5))=ges_so4_a05(j,i,k)
                all_loc(j,i,kchem( 6))=ges_so4_a06(j,i,k)
                all_loc(j,i,kchem( 7))=ges_so4_a07(j,i,k)
                all_loc(j,i,kchem( 8))=ges_so4_a08(j,i,k)

                all_loc(j,i,kchem( 9))=ges_no3_a01(j,i,k)
                all_loc(j,i,kchem(10))=ges_no3_a02(j,i,k)
                all_loc(j,i,kchem(11))=ges_no3_a03(j,i,k)
                all_loc(j,i,kchem(12))=ges_no3_a04(j,i,k)
                all_loc(j,i,kchem(13))=ges_no3_a05(j,i,k)
                all_loc(j,i,kchem(14))=ges_no3_a06(j,i,k)
                all_loc(j,i,kchem(15))=ges_no3_a07(j,i,k)
                all_loc(j,i,kchem(16))=ges_no3_a08(j,i,k)

                all_loc(j,i,kchem(17))=ges_nh4_a01(j,i,k)
                all_loc(j,i,kchem(18))=ges_nh4_a02(j,i,k)
                all_loc(j,i,kchem(19))=ges_nh4_a03(j,i,k)
                all_loc(j,i,kchem(20))=ges_nh4_a04(j,i,k)
                all_loc(j,i,kchem(21))=ges_nh4_a05(j,i,k)
                all_loc(j,i,kchem(22))=ges_nh4_a06(j,i,k)
                all_loc(j,i,kchem(23))=ges_nh4_a07(j,i,k)
                all_loc(j,i,kchem(24))=ges_nh4_a08(j,i,k)

                all_loc(j,i,kchem(25))=ges_ocx_a01(j,i,k)
                all_loc(j,i,kchem(26))=ges_ocx_a02(j,i,k)
                all_loc(j,i,kchem(27))=ges_ocx_a03(j,i,k)
                all_loc(j,i,kchem(28))=ges_ocx_a04(j,i,k)
                all_loc(j,i,kchem(29))=ges_ocx_a05(j,i,k)
                all_loc(j,i,kchem(30))=ges_ocx_a06(j,i,k)
                all_loc(j,i,kchem(31))=ges_ocx_a07(j,i,k)
                all_loc(j,i,kchem(32))=ges_ocx_a08(j,i,k)

                all_loc(j,i,kchem(33))=ges_bcx_a01(j,i,k)
                all_loc(j,i,kchem(34))=ges_bcx_a02(j,i,k)
                all_loc(j,i,kchem(35))=ges_bcx_a03(j,i,k)
                all_loc(j,i,kchem(36))=ges_bcx_a04(j,i,k)
                all_loc(j,i,kchem(37))=ges_bcx_a05(j,i,k)
                all_loc(j,i,kchem(38))=ges_bcx_a06(j,i,k)
                all_loc(j,i,kchem(39))=ges_bcx_a07(j,i,k)
                all_loc(j,i,kchem(40))=ges_bcx_a08(j,i,k)

                all_loc(j,i,kchem(41))=ges_clx_a01(j,i,k)
                all_loc(j,i,kchem(42))=ges_clx_a02(j,i,k)
                all_loc(j,i,kchem(43))=ges_clx_a03(j,i,k)
                all_loc(j,i,kchem(44))=ges_clx_a04(j,i,k)
                all_loc(j,i,kchem(45))=ges_clx_a05(j,i,k)
                all_loc(j,i,kchem(46))=ges_clx_a06(j,i,k)
                all_loc(j,i,kchem(47))=ges_clx_a07(j,i,k)
                all_loc(j,i,kchem(48))=ges_clx_a08(j,i,k)

                all_loc(j,i,kchem(49))=ges_nax_a01(j,i,k)
                all_loc(j,i,kchem(50))=ges_nax_a02(j,i,k)
                all_loc(j,i,kchem(51))=ges_nax_a03(j,i,k)
                all_loc(j,i,kchem(52))=ges_nax_a04(j,i,k)
                all_loc(j,i,kchem(53))=ges_nax_a05(j,i,k)
                all_loc(j,i,kchem(54))=ges_nax_a06(j,i,k)
                all_loc(j,i,kchem(55))=ges_nax_a07(j,i,k)
                all_loc(j,i,kchem(56))=ges_nax_a08(j,i,k)

                all_loc(j,i,kchem(57))=ges_oin_a01(j,i,k)
                all_loc(j,i,kchem(58))=ges_oin_a02(j,i,k)
                all_loc(j,i,kchem(59))=ges_oin_a03(j,i,k)
                all_loc(j,i,kchem(60))=ges_oin_a04(j,i,k)
                all_loc(j,i,kchem(61))=ges_oin_a05(j,i,k)
                all_loc(j,i,kchem(62))=ges_oin_a06(j,i,k)
                all_loc(j,i,kchem(63))=ges_oin_a07(j,i,k)
                all_loc(j,i,kchem(64))=ges_oin_a08(j,i,k)

                all_loc(j,i,kchem(65))=ges_num_a01(j,i,k)
                all_loc(j,i,kchem(66))=ges_num_a02(j,i,k)
                all_loc(j,i,kchem(67))=ges_num_a03(j,i,k)
                all_loc(j,i,kchem(68))=ges_num_a04(j,i,k)
                all_loc(j,i,kchem(69))=ges_num_a05(j,i,k)
                all_loc(j,i,kchem(70))=ges_num_a06(j,i,k)
                all_loc(j,i,kchem(71))=ges_num_a07(j,i,k)
                all_loc(j,i,kchem(72))=ges_num_a08(j,i,k)

                all_loc(j,i,kchem(73))=ges_h2o_a01(j,i,k)
                all_loc(j,i,kchem(74))=ges_h2o_a02(j,i,k)
                all_loc(j,i,kchem(75))=ges_h2o_a03(j,i,k)
                all_loc(j,i,kchem(76))=ges_h2o_a04(j,i,k)
                all_loc(j,i,kchem(77))=ges_h2o_a05(j,i,k)
                all_loc(j,i,kchem(78))=ges_h2o_a06(j,i,k)
                all_loc(j,i,kchem(79))=ges_h2o_a07(j,i,k)
                all_loc(j,i,kchem(80))=ges_h2o_a08(j,i,k)
                end if
             end if
            !!! cwy mosaic ------------------------------------------------ chem
