...
!!! cwy mosaic ----------------------------------
  public :: rmin_value, sf_pm2p5, sf_pm10, sf_aodce318, sf_aodmodis, pm10bufr_only_coarse
  public :: sf_srf_eabs, sf_srf_esca, sf_srf_eext
  public :: sf_jac, corr_length, use_interbin_filter
  public :: nsize_mosaic_wrf, n_cvmas, n_cvnum, n_cvaer
  public :: limit_srfeext
  public :: i_srfeabs_ob_type
  public :: i_srfesca_ob_type
  public :: i_srfeext_ob_type
  public :: mype_eabs,iout_eabs,nleabs
  public :: mype_esca,iout_esca,nlesca
  public :: mype_eext,iout_eext,nleext
!!! cwy mosaic ----------------------------------
  public :: lsaveobsens
...
!!! cwy mosaic ----------------------------------------------
  integer(i_kind),parameter:: i_srfeabs_ob_type =39  ! srfeabs_ob_type
  integer(i_kind),parameter:: i_srfesca_ob_type =40  ! srfesca_ob_type
  integer(i_kind),parameter:: i_srfeext_ob_type =41  ! srfeext_ob_type
...
!!! cwy mosaic ----------------------------------
  real(r_kind)    :: rmin_value, sf_pm2p5, sf_pm10, sf_aodce318, sf_aodmodis, sf_srf_eabs, sf_srf_esca, sf_srf_eext
  real(r_kind)    :: sf_jac, corr_length
  integer(i_kind) :: nsize_mosaic_wrf, n_cvmas, n_cvnum, n_cvaer
  integer(i_kind) :: pm10bufr_only_coarse, limit_srfeext, use_interbin_filter
  integer(i_kind) :: nleabs, iout_eabs, mype_eabs
  integer(i_kind) :: nlesca, iout_esca, mype_esca
  integer(i_kind) :: nleext, iout_eext, mype_eext
!!! cwy mosaic ----------------------------------
...
!!! cwy mosaic ----------------------------------
    iout_eabs=238  ! aerosol product (aod)
    iout_esca=239  ! aerosol product (aod)
    iout_eext=240  ! aerosol product (aod)
!!! cwy mosaic ----------------------------------
...
!!! cwy mosaic ----------------------------------
    mype_eabs= max(0,npe-13) ! aerosol product (aod)
    mype_esca= max(0,npe-13) ! aerosol product (aod)
    mype_eext= max(0,npe-13) ! aerosol product (aod)
!!! cwy mosaic ----------------------------------
...
!!! cwy mosaic -----------------------------------------------------
    cobstype( i_srfeabs_ob_type)="srf aerosol eabs    " ! srfeabs_ob_type
    cobstype( i_srfesca_ob_type)="srf aerosol esca    " ! srfesca_ob_type
    cobstype( i_srfeext_ob_type)="srf aerosol eext    " ! srfeext_ob_type
!!! cwy mosaic -----------------------------------------------------
...
!!! cwy mosaic ----------------------------------
    ...
    n_cvnum=0
    n_cvaer=0
    use_interbin_filter=0
    limit_srfeext=1
    sf_srf_eabs=1.
    sf_srf_esca=1.
    sf_srf_eext=1.
    pm10bufr_only_coarse=0
!!! cwy mosaic ----------------------------------
...
!$$$ end documentation block
    ...
    if (l4dvar) then                              !!! l4dvar=F
       offtime_data = .true.   ! .true. = ignore difference in obs ref time
    endif
    if (l4dvar .or. lsqrtb .or. lbicg) then       !!! lsqrtb=F, lbicg=F
       luse_obsdiag = .true.
       if (mype==0) then
          write(6,*)'NOTICE: resetting luse_obsdiag to true '
       endif
    endif
    if(.not. luse_obsdiag) then                   !!! luse_obsdiag=F
    ...
    if(luse_obsdiag)then                          !!! luse_obsdiag=F
