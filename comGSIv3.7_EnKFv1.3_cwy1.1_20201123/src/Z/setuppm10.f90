...
!!! cwy mosaic4 -------------------------------------- chem
      wt_mas_a01, wt_mas_a02, wt_mas_a03, wt_mas_a04
!!! cwy mosaic4 -------------------------------------- chem
...
!!! cwy mosaic4 -------------------------------------- chem
      naero_mosaic4_wrf, aeronames_mosaic4_wrf, laeroana_mosaic4
  use obsmod, only: sf_pm10, pm10bufr_only_a04
!!! cwy mosaic4 -------------------------------------- chem
...
!!! cwy mosaic4 -------------------------------------- chem
  integer(i_kind) :: n_mosaic4_var
  real(r_kind),allocatable,dimension(:,:,:,:) :: ges_num_a01
  real(r_kind),allocatable,dimension(:,:,:,:) :: ges_num_a02
  real(r_kind),allocatable,dimension(:,:,:,:) :: ges_num_a03
  real(r_kind),allocatable,dimension(:,:,:,:) :: ges_num_a04
  real(r_kind) :: num_a01_ges, num_a02_ges, num_a03_ges, num_a04_ges

  real(r_kind),allocatable,dimension(:,:,:,:) :: ges_pmx_a01
  real(r_kind),allocatable,dimension(:,:,:,:) :: ges_pmx_a02
  real(r_kind),allocatable,dimension(:,:,:,:) :: ges_pmx_a03
  real(r_kind),allocatable,dimension(:,:,:,:) :: ges_pmx_a04
  real(r_kind) :: pmx_a01_ges, pmx_a02_ges, pmx_a03_ges, pmx_a04_ges

  real(r_kind) :: wt_a01_ges, wt_a02_ges, wt_a03_ges, wt_a04_ges
!!! cwy mosaic4 -------------------------------------- chem
...
!!! cwy mosaic4 -------------------------------------- chem
  if( wrf_mass_regional .and. laeroana_mosaic4 ) then
    call gsi_chemguess_get( 'aerosols::3d',n_mosaic4_var,ier )
    if( n_mosaic4_var/=naero_mosaic4_wrf ) then
      write(6,*) 'setuppm10: not all mosaic4 aerosols in anavinfo'
      call stop2(451)
    end if

    do i=1, naero_mosaic4_wrf
      aeroname=upper2lower(aeronames_mosaic4_wrf(i))
      call gsi_chemguess_get('var::'//trim(aeroname), ipm10, ier)
      if( ier>0 .or. ipm10<=0 ) then
        write(6,*) 'convinfo: '//trim(aeroname)//' missing in anavinfo'
        call stop2(452)
...
!!! -------------------- mosaic4 pmx type I
    if( .false. ) then
    aeroname='so4_a01'
    call gsi_bundlegetpointer(gsi_chemguess_bundle(1),trim(aeroname),rank3,ier)
    if( ier==0 ) then
      allocate(ges_pm10(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))
      ges_pm10(:,:,:,1)=rank3
      do ifld=2, nfldsig
        call gsi_bundlegetpointer(gsi_chemguess_bundle(ifld),trim(aeroname),rank3,ier)
        ges_pm10(:,:,:,ifld)=rank3
      ...
      write(6,*) 'setuppm10: '//trim(aeroname)//' no in chem bundle, ier=', ier
      call stop2(453)
    end if
...
!!! -------------------- 
      allocate(ges_num_a01(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))
      allocate(ges_num_a02(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))
      allocate(ges_num_a03(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))
      allocate(ges_num_a04(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))

      allocate(ges_pmx_a01(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))
      allocate(ges_pmx_a02(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))
      allocate(ges_pmx_a03(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))
      allocate(ges_pmx_a04(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))

      aeroname='num_a01'
      call gsi_bundlegetpointer(gsi_chemguess_bundle(1),trim(aeroname),rank3,ier)
      if( ier==0 ) then
        ges_num_a01(:,:,:,1)=rank3    !!! log(#/kg)
      else
        write(6,*) 'setuppm10: '//trim(aeroname)//' no in chem bundle, ier=', ier
        call stop2(453)
      end if
...
!!! ----------------------- mosaic4 pmx type II
      ges_pmx_a01(:,:,:,1)=exp(ges_num_a01(:,:,:,1))*wt_mas_a01 ! ug/kg
      ges_pmx_a02(:,:,:,1)=exp(ges_num_a02(:,:,:,1))*wt_mas_a02
      ges_pmx_a03(:,:,:,1)=exp(ges_num_a03(:,:,:,1))*wt_mas_a03
      ges_pmx_a04(:,:,:,1)=exp(ges_num_a04(:,:,:,1))*wt_mas_a04
!!! ----------------------- 
  end if

    else
      write(6,*) 'setuppm10: inconsistent vector sizes'
      call stop2(420)
    end if
...
!might convert for cmaq at some point as well
        ...
        if (wrf_mass_regional) then
           ...
           data(ierror,i)=data(ierror,i)/(ps_ges*r1000/(rd*tv_ges))   !!!  !ug/m3->ug/kg
!!! cwy mosaic4 -------------------------------- chem
            if( laeroana_mosaic4 ) then
              if( conc==0 ) conc=tiny_r_kind
              conc=log(conc)
              if( data(ierror,i)<=0 ) data(ierror,i)=tiny_r_kind
              !data(ierror,i)=0.5559*log(data(ierror,i))-0.8058
              data(ierror,i)=sf_pm10*log(data(ierror,i))
            end if
!!! cwy mosaic4 -------------------------------- chem
        endif
...
!!! cwy mosaic4 ------------------------------------------ chem
        if( laeroana_mosaic4 ) then
          if( .false. ) then
            ...
            call tintrp2a11(ges_pmx_a01,pmx_a01_ges,dlat,dlon,dtime,hrdifsig,&
            mype,nfldsig)
            call tintrp2a11(ges_pmx_a02,pmx_a02_ges,dlat,dlon,dtime,hrdifsig,&
            mype,nfldsig)
            call tintrp2a11(ges_pmx_a03,pmx_a03_ges,dlat,dlon,dtime,hrdifsig,&
            mype,nfldsig)
            call tintrp2a11(ges_pmx_a04,pmx_a04_ges,dlat,dlon,dtime,hrdifsig,&
            mype,nfldsig)

            if( pm10bufr_only_a04==1 ) then
              pm10ges=pmx_a04_ges !!! ug/kg
            else
              pm10ges=pmx_a01_ges+pmx_a02_ges+pmx_a03_ges+pmx_a04_ges !!! ug/kg
            end if
          end if

          if( pm10ges<=0 ) pm10ges=tiny_r_kind
          pm10ges=log(pm10ges)

          if( .true. ) then
          call tintrp2a11(ges_num_a01,num_a01_ges,dlat,dlon,dtime,hrdifsig, &
          mype,nfldsig)
          call tintrp2a11(ges_num_a02,num_a02_ges,dlat,dlon,dtime,hrdifsig, &
          mype,nfldsig)
          call tintrp2a11(ges_num_a03,num_a03_ges,dlat,dlon,dtime,hrdifsig, &
          mype,nfldsig)
          call tintrp2a11(ges_num_a04,num_a04_ges,dlat,dlon,dtime,hrdifsig, &
          mype,nfldsig)

          call tintrp2a11(wt_mas_a01,wt_a01_ges,dlat,dlon,dtime,hrdifsig,&
          mype,nfldsig)
          call tintrp2a11(wt_mas_a02,wt_a02_ges,dlat,dlon,dtime,hrdifsig,&
          mype,nfldsig)
          call tintrp2a11(wt_mas_a03,wt_a03_ges,dlat,dlon,dtime,hrdifsig,&
          mype,nfldsig)
          call tintrp2a11(wt_mas_a04,wt_a04_ges,dlat,dlon,dtime,hrdifsig,&
          mype,nfldsig)
          end if
        end if
!!! cwy mosaic4 ------------------------------------------ chem
        ...
        innov = conc - pm10ges
        write(*,*) 'cwy obs, sim, innov=conc-pm10ges in log(ug/kg)=', conc, pm10ges, innov
...
!!! cwy mosaic4 ---------------------------------- chem
          if( laeroana_mosaic4 ) then
            if( .true. ) then
              if( pm10bufr_only_a04==1 ) then
                my_head%dpmx_dnum(1)=0.
                my_head%dpmx_dnum(2)=0.
                my_head%dpmx_dnum(3)=0.
                my_head%dpmx_dnum(4)=exp(num_a04_ges)/exp(pm10ges)*wt_a04_ges
              else
                my_head%dpmx_dnum(1)=exp(num_a01_ges)/exp(pm10ges)*wt_a01_ges
                my_head%dpmx_dnum(2)=exp(num_a02_ges)/exp(pm10ges)*wt_a02_ges
                my_head%dpmx_dnum(3)=exp(num_a03_ges)/exp(pm10ges)*wt_a03_ges
                my_head%dpmx_dnum(4)=exp(num_a04_ges)/exp(pm10ges)*wt_a04_ges
              end if
            else
              if( pm10bufr_only_a04==1 ) then
                ...
                my_head%dpmx_dnum(2)=0.
                my_head%dpmx_dnum(3)=0.
                my_head%dpmx_dnum(4)=1.
              else
                my_head%dpmx_dnum(1)=1.
                my_head%dpmx_dnum(2)=1.
                my_head%dpmx_dnum(3)=1.
                my_head%dpmx_dnum(4)=1.
              end if
            end if
          end if
...
!  
  ...
  use chemmod, only: naero_gocart_wrf,aeronames_gocart_wrf
!!! cwy mosaic4 ------------------------------------------ chem
  use chemmod, only: naero_mosaic4_wrf,aeronames_mosaic4_wrf
!!! cwy mosaic4 ------------------------------------------ chem
...
!
  if (wrf_mass_regional .and. laeroana_gocart) then
     ...
     enddo
!!! cwy mosaic4 ------------------------------------------ chem
  else if(wrf_mass_regional .and. laeroana_mosaic4) then 
      do i=1, naero_mosaic4_wrf
        aeroname=upper2lower(aeronames_mosaic4_wrf(i))
        call gsi_chemguess_get('var::'//trim(aeroname),ivar,istatus)
        if( ivar==0 ) exit
...
!!! cwy mosaic4 ------------------------------- chem
    if(allocated(ges_num_a01)) deallocate(ges_num_a01)
    if(allocated(ges_num_a02)) deallocate(ges_num_a02)
    if(allocated(ges_num_a03)) deallocate(ges_num_a03)
    if(allocated(ges_num_a04)) deallocate(ges_num_a04)
    if(allocated(ges_pmx_a01)) deallocate(ges_pmx_a01)
    if(allocated(ges_pmx_a02)) deallocate(ges_pmx_a02)
    if(allocated(ges_pmx_a03)) deallocate(ges_pmx_a03)
    if(allocated(ges_pmx_a04)) deallocate(ges_pmx_a04)
!!! cwy mosaic4 ------------------------------- chem
