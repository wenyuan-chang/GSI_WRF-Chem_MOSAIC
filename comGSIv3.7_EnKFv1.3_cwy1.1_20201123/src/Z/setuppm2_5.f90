...
!!! cwy mosaic4 ------------------------------------- chem
    wt_mas_a01, wt_mas_a02, wt_mas_a03
!!! cwy mosaic4 ------------------------------------- chem
...
!!! cwy mosaic4 ------------------------------------- chem
  ...
  real(r_kind) :: num_a01_ges, num_a02_ges, num_a03_ges
  ...
  real(r_kind) :: pmx_a01_ges, pmx_a02_ges, pmx_a03_ges

  real(r_kind) :: wt_a01_ges, wt_a02_ges, wt_a03_ges
!!! cwy mosaic4 ------------------------------------- chem
...
!!! cwy mosaic4 ------------------------------------- chem
  if( wrf_mass_regional .and. laeroana_mosaic4 ) then
    ...
    if( n_mosaic4_var/=naero_mosaic4_wrf ) then
      write(6,*) 'setuppm2_5: not all mosaic4 aerosols in anavinfo'
      call stop2(451)
    end if
...
!!! --------------------- mosaic4 pmx type I
    if( .false. ) then
      ...
      if( ier==0 ) then
        allocate(ges_pm2_5(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))
        ges_pm2_5(:,:,:,1)=rank3
        do ifld=2, nfldsig
          call gsi_bundlegetpointer(gsi_chemguess_bundle(ifld),trim(aeroname),rank3,ier)
          ges_pm2_5(:,:,:,ifld)=rank3
        ...
        write(6,*) 'setuppm2_5: '//trim(aeroname)//' no in chem bundle, ier=',ier
        call stop2(453)
      end if
...
!!! --------------------- mosaic4 pmx type II
      aeroname='so4_a01'
      call gsi_bundlegetpointer(gsi_chemguess_bundle(1),trim(aeroname),rank3,ier)
!!!write(*,*) 'cwy size(rank3)=', size(rank3,1), size(rank3,2), size(rank3,3), nfldsig  ! 46, 46, 40, 1
...
!!! ---------------------
      ...
      write(6,*) 'setuppm2_5: inconsistent vector sizes'
      call stop2(420)
    end if
...
!!!          write(*,*) 'cwy dup(k), dup(l)=', dup(k), dup(l)
        end if
     end do
  end do
...
!might convert for cmaq at some point as well
        ...
        if (wrf_mass_regional) then
           ...
           call tintrp2a11(ges_tv(:,:,1,nfldsig),tv_ges,dlat,dlon,dtime,hrdifsig,&
                mype,nfldsig)
!!! cwy add ---------------
           if( .not.wrf_pm2_5 ) then
           conc=conc/(ps_ges*r1000/(rd*tv_ges))   !!! cwy: ug/m3->ug/kg, ps_ges in cbar
           data(ierror,i)=data(ierror,i)/(ps_ges*r1000/(rd*tv_ges))   !!! cwy: ug/m3->ug/kg
!!! cwy mosaic4 ------------------------------------- chem
              if( laeroana_mosaic4 ) then
                if( conc<=0 ) conc=tiny_r_kind
                conc=log(conc)
                if( data(ierror,i)<=0 ) data(ierror,i)=tiny_r_kind
                !data(ierror,i)=0.5492*log(data(ierror,i))-0.6066
                data(ierror,i)=sf_pm2p5*log(data(ierror,i))
              end if
!!! cwy mosaic4 ------------------------------------- chem
           end if
!!! cwy add ---------------
...
!!! cwy mosaic4 ------------------------------------- chem
           if( laeroana_mosaic4 ) then
              if( .false. ) then
                ...
                mype,nfldsig)
                pm2_5ges=pmx_a01_ges+pmx_a02_ges+pmx_a03_ges    !!! ug/kg
              end if

              if( pm2_5ges<=0 ) pm2_5ges=tiny_r_kind
              pm2_5ges=log(pm2_5ges)
...
!!! cwy mosaic4 ------------------------------------- chem
           ...
           innov = conc - pm2_5ges
           write(*,*) 'cwy obs, sim, innov=conc-pm2_5ges, obserr in log(ug/kg) =', conc, pm2_5ges, innov, data(ierror,i)
        end if
...
!!! cwy mosaic4 -------------------------------- chem
           if( laeroana_mosaic4 ) then
           my_head%dpmx_dnum(1)=exp(num_a01_ges)/exp(pm2_5ges)*wt_a01_ges
           my_head%dpmx_dnum(2)=exp(num_a02_ges)/exp(pm2_5ges)*wt_a02_ges
           my_head%dpmx_dnum(3)=exp(num_a03_ges)/exp(pm2_5ges)*wt_a03_ges
           end if
!!! cwy mosaic4 -------------------------------- chem
