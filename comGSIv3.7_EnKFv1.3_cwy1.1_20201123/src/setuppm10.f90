...
!!! cwy mosaic -------------------------------------- chem
!!!   naero_mosaic_wrf, aeronames_mosaic_wrf
      laeroana_mosaic
  use obsmod, only: sf_pm10, pm10bufr_only_coarse
  use obsmod, only: nsize_mosaic_wrf
  use obsmod, only: n_cvmas, n_cvaer
!!! cwy mosaic -------------------------------------- chem
...
!!! cwy mosaic -------------------------------------- chem
  ...
  integer            :: ksiz, ksiz_min, kss
  character( len=2 ) :: c2

  if( nsize_mosaic_wrf==4 ) ksiz_min=4
  if( nsize_mosaic_wrf==8 ) ksiz_min=7
...
!!! cwy mosaic -------------------------------------- chem
  if( wrf_mass_regional .and. laeroana_mosaic ) then
    if( pm10bufr_only_coarse==1 ) then
      kss=ksiz_min
    else
      kss=1
    end if
    ...
    !
    if( n_cvmas/=0 ) then
      do ksiz=kss, nsize_mosaic_wrf
        write(c2,'(i2.2)') ksiz
        call gsi_bundlegetpointer(gsi_chemguess_bundle(1),'mas_a'//c2,rank3,ier)
        if( ier==0 ) then
          if( ksiz==kss ) then
            allocate(ges_pm10(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))
            ges_pm10(:,:,:,1)=rank3
!!!            do ifld=2, nfldsig
!!!              call gsi_bundlegetpointer(gsi_chemguess_bundle(ifld),'mas_a'//c2,rank3,ier)
!!!              ges_pm10(:,:,:,ifld)=rank3
!!!            end do
          else
            ges_pm10(:,:,:,1)=ges_pm10(:,:,:,1)+rank3
...
!!!            end do
    ...
    if( n_cvaer/=0 ) then
      do ksiz=kss, nsize_mosaic_wrf
        write(c2,'(i2.2)') ksiz
        call gsi_bundlegetpointer(gsi_chemguess_bundle(1),'so4_a'//c2,rank3,ier)
        if( ier==0 ) then
          if( ksiz==kss ) then
            allocate(ges_pm10(size(rank3,1),size(rank3,2),size(rank3,3),nfldsig))
            ges_pm10(:,:,:,1)=rank3
          ...
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),'no3_a'//c2,rank3,ier)
          ges_pm10(:,:,:,1)=ges_pm10(:,:,:,1)+rank3
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),'nh4_a'//c2,rank3,ier)
          ges_pm10(:,:,:,1)=ges_pm10(:,:,:,1)+rank3
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),'oc_a'//c2,rank3,ier)
          ges_pm10(:,:,:,1)=ges_pm10(:,:,:,1)+rank3
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),'bc_a'//c2,rank3,ier)
          ges_pm10(:,:,:,1)=ges_pm10(:,:,:,1)+rank3
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),'oin_a'//c2,rank3,ier)
          ges_pm10(:,:,:,1)=ges_pm10(:,:,:,1)+rank3
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),'cl_a'//c2,rank3,ier)
          ges_pm10(:,:,:,1)=ges_pm10(:,:,:,1)+rank3
          call gsi_bundlegetpointer(gsi_chemguess_bundle(1),'na_a'//c2,rank3,ier)
          ges_pm10(:,:,:,1)=ges_pm10(:,:,:,1)+rank3
        end if
      end do
    end if
...
!!! cwy mosaic -------------------------------- chem
            if( laeroana_mosaic ) then
              data(ierror,i)=sf_pm10*data(ierror,i)
            end if
!!! cwy mosaic -------------------------------- chem
        endif
...
!!! cwy mosaic ------------------------------------------ chem
        if( laeroana_mosaic ) then
          call tintrp2a11(ges_pm10,pm10ges,dlat,dlon,dtime,hrdifsig,mype,nfldsig)
        end if
!!! cwy mosaic ------------------------------------------ chem
...
!  
  ...
  use chemmod, only: naero_gocart_wrf,aeronames_gocart_wrf
!!! cwy mosaic ------------------------------------------ chem
!  use chemmod, only: naero_mosaic_wrf,aeronames_mosaic_wrf
!!! cwy mosaic ------------------------------------------ chem
...
!
  if (wrf_mass_regional .and. laeroana_gocart) then
     ...
     enddo
!!! cwy mosaic ------------------------------------------ chem
!  else if(wrf_mass_regional .and. laeroana_mosaic) then 
!      do i=1, naero_mosaic_wrf
!        aeroname=upper2lower(aeronames_mosaic_wrf(i))
!        call gsi_chemguess_get('var::'//trim(aeroname),ivar,istatus)
!        if( ivar==0 ) exit
!      end do
!!! cwy mosaic ------------------------------------------ chem
