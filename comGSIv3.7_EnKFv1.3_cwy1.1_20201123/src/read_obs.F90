...
!!! cwy mosaic -----------------------------------------
       else if(trim(dtype) == 'ce318_aod' ) then
          lexist = .false.
          fileloopce318_aod:do while(ireadmg(lnbufr,subset,idate2) >= 0)
            do while(ireadsb(lnbufr)>=0)
              if (subset == 'NC008041') then
                lexist=.true.
                exit fileloopce318_aod
              else
                cycle
              end if
            end do
            nread = nread + 1
          end do fileloopce318_aod

          if( lexist ) then
            write(6,*) 'found ce318_aod in bufr'
          else
            write(6,*) 'did not find ce318_aod in bufr'
          endif
       else if(trim(dtype)=='srf_eabs') then
          lexist = .false.
          fileloopsrfeabs:do while(ireadmg(lnbufr,subset,idate2) >= 0)
            do while(ireadsb(lnbufr)>=0)
              if (subset == 'NC008041') then
                lexist=.true.
                exit fileloopsrfeabs
              else
                cycle
              end if
            end do
            nread = nread + 1
          end do fileloopsrfeabs

          if( lexist ) then
            write(6,*) 'found srf_eabs in bufr'
          else
            write(6,*) 'did not find srf_eabs in bufr'
          endif

       else if(trim(dtype)=='srf_esca') then
          lexist = .false.
          fileloopsrfesca:do while(ireadmg(lnbufr,subset,idate2) >= 0)
            do while(ireadsb(lnbufr)>=0)
              if (subset == 'NC008041') then
                lexist=.true.
                exit fileloopsrfesca
              else
                cycle
              end if
            end do
            nread = nread + 1
          end do fileloopsrfesca

          if( lexist ) then
            write(6,*) 'found srf_esca in bufr'
          else
            write(6,*) 'did not find srf_esca in bufr'
          endif

       else if(trim(dtype)=='srf_eext') then
          lexist = .false.
          fileloopsrfeext:do while(ireadmg(lnbufr,subset,idate2) >= 0)
            do while(ireadsb(lnbufr)>=0)
              if (subset == 'NC008041') then
                lexist=.true.
                exit fileloopsrfeext
              else
                cycle
              end if
            end do
            nread = nread + 1
          end do fileloopsrfeext

          if( lexist ) then
            write(6,*) 'found srf_eext in bufr'
          else
            write(6,*) 'did not find srf_eext in bufr'
          endif
!!! cwy mosaic -----------------------------------------
...
!   Set analysis time and allocate/initialize arrays and variables
    ...
    if(njqc) then   !!! njqc=F
       ...
       call converr_read(mype)  !!! converr.f90 read errtable
    endif
...
!   type type of GPS data (if present)
    ...
    do i=1,ndat
       ...
       if (obstype == 't'  .or. obstype == 'uv' .or. &
          ...
          ditype(i) = 'gps'
!!! cwy mosaic --------------------------------------
       else if ( obstype == 'modis_aod' ) then
          ditype(i) = 'aero'
       else if ( obstype == 'ce318_aod' ) then
          ditype(i) = 'ce318_aod'
       else if ( obstype == 'srf_eabs' ) then
          ditype(i) = 'srf_eabs'
       else if ( obstype == 'srf_esca' ) then
          ditype(i) = 'srf_esca'
       else if ( obstype == 'srf_eext' ) then
          ditype(i) = 'srf_eext'
!!! cwy mosaic --------------------------------------
...
!   Check info files to see if data is read.
       ...
       else if(ditype(i) == 'ce318_aod')then
          if(diag_aero .and. .not. reduce_diag)minuse=-2
          do j=1,jpch_aero
             if(trim(dsis(i)) == trim(nusis_aero(j)) .and. iuse_aero(j) > minuse)nuse=.true.
          end do
...
!         Process aerosol data
             ...
             call read_aerosol_ce318(nread,npuse,nouse,&
                  infile,obstype,gstime,lunout,twind,sis,nobs_sub1(1,i))
             string='READ_AEROSOL_CE318'
          else if (ditype(i) == 'srf_eabs' )then
             call read_aerosol_srfeabs(nread,npuse,nouse,&
                  infile,obstype,gstime,lunout,twind,sis,nobs_sub1(1,i))
             string='READ_AEROSOL_SRFEABS'
          else if (ditype(i) == 'srf_esca' )then
             call read_aerosol_srfesca(nread,npuse,nouse,&
                  infile,obstype,gstime,lunout,twind,sis,nobs_sub1(1,i))
             string='READ_AEROSOL_SRFESCA'
          else if (ditype(i) == 'srf_eext' )then
             call read_aerosol_srfeext(nread,npuse,nouse,&
                  infile,obstype,gstime,lunout,twind,sis,nobs_sub1(1,i))
             string='READ_AEROSOL_SRFEEXT'
!!! cwy mosaic --------------------------------------
