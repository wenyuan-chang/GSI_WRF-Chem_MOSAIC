...
!
subroutine stplimnum(rval,sval,sges,outmin,outmax,nstep,itbin,cbin)
  ...
  integer(i_kind)                                   :: nstep, itbin
  real(r_kind), dimension(max(1,nstep)), intent(in) :: sges
  real(r_quad), dimension(max(1,nstep)), intent(out):: outmin, outmax
  character( len=* ), intent( in )                  :: cbin

  integer(i_kind)                                   :: i, j, k, kk, n, ier, istatus
  real(r_kind)                                      :: aernum, aernum_x    ! used in log(#/kg)
  real(r_kind)                                      :: rv
  ...
  ier=0
  call gsi_bundlegetpointer(sval,trim(cbin),sq,istatus);ier=istatus+ier
  call gsi_bundlegetpointer(rval,trim(cbin),rq,istatus);ier=istatus+ier
  if( ier/=0 )return
  call gsi_bundlegetpointer(gsi_chemguess_bundle(itbin),trim(cbin),ges_num_it,ier)
  if( ier/=0 )return
  ...
  if( n>nc3d ) then 
    print *, 'cwy-stplimnum: cannot find '//trim(cbin)//' in stplimnum'
    return
  end if
  ...
  if( nstep>0 ) then
    ...
    do i=2, lat1+1
      ...
      do kk=1, nstep
        aernum_x=aernum+sges(kk)*rq(i,j,k)
        if( aernum_x<=scalemin*ges_num_it(i,j,k) ) then 
          rv=ges_num_it(i,j,k)-aernum_x
          !outmin(kk)=outmin(kk)+factmin_num*rv*rv/ges_num_it(i,j,k)**2
          outmin(kk)=outmin(kk)+factmin_num*rv*rv/dssv(i,j,k,n)**2
        else if( aernum_x>=scalemax*ges_num_it(i,j,k) ) then 
          rv=aernum_x-ges_num_it(i,j,k)
          !outmax(kk)=outmax(kk)+factmax_num*rv*rv/ges_num_it(i,j,k)**2
          outmax(kk)=outmax(kk)+factmax_num*rv*rv/dssv(i,j,k,n)**2
        end if
      end do
    end do
    end do
