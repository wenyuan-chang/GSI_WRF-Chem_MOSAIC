...
!
subroutine intlimnum(rval,sval,itbin,cbin)
  use gridmod, only: nsig, lat1, lon1
  use jfunc, only: factmin_num, factmax_num, scalemin, scalemax
  use gsi_chemguess_mod, only: gsi_chemguess_bundle
  use berror, only: dssv                          ! #/kg
  use control_vectors, only: cvars3d, nc3d
  implicit none

  type(gsi_bundle), intent( in )    ::  sval
  type(gsi_bundle), intent( inout ) ::  rval
  integer, intent(in)               ::  itbin
  character( len=* ), intent( in )  ::  cbin

  integer(i_kind)                   ::  i, j, k, n, ier, istatus
  real(r_kind)                      ::  aernum

  real(r_kind), pointer, dimension(:,:,:) ::  sq=>NULL()
  real(r_kind), pointer, dimension(:,:,:) ::  rq=>NULL()
  real(r_kind), pointer, dimension(:,:,:) ::  ges_num_it=>NULL()

  ier=0
  call gsi_bundlegetpointer(sval,trim(cbin),sq,istatus);ier=istatus+ier !  log(#/kg)
  call gsi_bundlegetpointer(rval,trim(cbin),rq,istatus);ier=istatus+ier !  log(#/kg)
  if( ier/=0 )return
  call gsi_bundlegetpointer(gsi_chemguess_bundle(itbin),trim(cbin),ges_num_it,ier) ! log(#/kg)
  if( ier/=0 )return

  do n=1, nc3d
    if( trim(cvars3d(n))==trim(cbin) ) exit
  end do
  if( n>nc3d ) then
    if( factmin_num==0 .and. factmax_num==0 ) return
    print *, 'cwy-factmin_num, factmax_num=', factmin_num, factmax_num
    print *, 'cwy-intlimnum: cannot find '//trim(cbin)//' in intlimnum'
    stop
  end if
  !print *, 'cwy intlimnum scalemin, scalemax=', scalemin, scalemax, trim(cbin)

  do k=1, nsig
  do j=2, lon1+1
  do i=2, lat1+1
    aernum=ges_num_it(i,j,k)+sq(i,j,k)
    if( aernum<=scalemin*ges_num_it(i,j,k) ) then
      !rq(i,j,k)=rq(i,j,k)+factmin_num*(ges_num_it(i,j,k)-aernum)/ges_num_it(i,j,k)**2
      rq(i,j,k)=rq(i,j,k)+factmin_num*(ges_num_it(i,j,k)-aernum)/dssv(i,j,k,n)**2
    else if( aernum>=scalemax*ges_num_it(i,j,k) ) then
      !rq(i,j,k)=rq(i,j,k)+factmax_num*(aernum-ges_num_it(i,j,k))/ges_num_it(i,j,k)**2
      rq(i,j,k)=rq(i,j,k)+factmax_num*(aernum-ges_num_it(i,j,k))/dssv(i,j,k,n)**2
    end if
  end do
  end do
  end do
