   subroutine setupaod_aaod_ce318(lunin,mype,nchanl,nreal,nobs,&
...
!$$$
  ...
  use obsmod, only: sf_aaod_ce318
  use m_obsdiags, only: aaodhead
  use m_aaodNode, only: aaodNode, aaodNode_typecast
  use obsmod, only: i_aaod_ob_type
  !!! cwy mosaic -------------------------- chem
...
! Declare local arrays
  ...
  type(aaodNode),pointer:: my_head
  type(obs_diag),pointer:: my_diag
  type(rad_obs_type) :: radmod
  character(len=*),parameter:: myname="setupaod_aaod_ce318"
...
! Initialize channel related information
  ...
  do j=1,jpch_aero
     if(isis == nusis_aero(j))then 
        jc=jc+1
        if(jc > nchanl)then
           write(6,*)'setupaod_aaod_ce318:  ***ERROR*** in channel numbers, jc,nchanl=',jc,nchanl,&
                '  ***STOP IN setupaod_aaod_ce318***'
           call stop2(71)
        end if
...
!       Set error instrument channels
  ...
  if ( mype == 0 .and. .not.l_may_be_passive) write(6,*)mype,'setupaod_aaod_ce318: passive obs',is,isis
  if(nchanl > jc) write(6,*)'setupaod_aaod_ce318:  channel number reduced for ', &
       obstype,nchanl,' --> ',jc
  if(jc == 0) then
     if(mype == 0) write(6,*)'setupaod_aaod_ce318: No channels found for ', &
          obstype,isis
     if(nobs > 0)read(lunin)
     return
  end if
  if (toss) then
     if(mype == 0)write(6,*)'setupaod_aaod_ce318: all obs var > 1e4.  do not use ',&
  ...
  write(*,*) 'cwy: setupaod_aaod_ce318.F90'
...
!    first outer iteration.
     if (init_pass .and. mype==mype_diaghdr(is)) then
        ...
        write(6,*)'setupaod_aaod_ce318:  write header record for ',&
...
!       Interpolate model fields to observation location, call crtm and create jacobians
        ...
        call call_aod_mosaic(dtime,data_s(:,n),nchanl,nreal,layer_od,zhgt,jacobian_aero,'aaod_ce318')
        else
...
!!! cwy mosaic ------------------------------------------------------------------------ chem
                if( laeroana_mosaic ) then
                  ...
                  tnoise(i)=sf_aaod_ce318*tnoise(i)/sum(zhgt(:))*1.e+6

                  if ( aod_obs(i) > zero ) then
                     write(6,'(A,3i6,4f8.3,2f8.2,i)') 'cwy, mype, iobs, ichan, aaodobs, aaodsim, innov, err, lat, lon : ',  &
...
!          Load data into output arrays
           if (icc > 0) then
              ...
              call obsLList_appendNode(aaodhead(ibin),my_node)
              my_node => null()
        ...
        if(luse_obsdiag)then
           ...
           do ii=1,nchanl
              if (.not.lobsdiag_allocated) then
                 if (.not.associated(obsdiags(i_aaod_ob_type,ibin)%head)) then
                    obsdiags(i_aaod_ob_type,ibin)%n_alloc = 0
                    allocate(obsdiags(i_aaod_ob_type,ibin)%head,stat=istat)
                    if (istat/=0) then
                       write(6,*)'setupaod_aaod_ce318: failure to allocate obsdiags',istat
                       call stop2(276)
                    end if
                    obsdiags(i_aaod_ob_type,ibin)%tail => obsdiags(i_aaod_ob_type,ibin)%head
                 else
                    allocate(obsdiags(i_aaod_ob_type,ibin)%tail%next,stat=istat)
                    if (istat/=0) then
                       write(6,*)'setupaod_aaod_ce318: failure to allocate obsdiags',istat
                       call stop2(277)
                    end if
                    obsdiags(i_aaod_ob_type,ibin)%tail => obsdiags(i_aaod_ob_type,ibin)%tail%next
                 end if
                 obsdiags(i_aaod_ob_type,ibin)%n_alloc = obsdiags(i_aaod_ob_type,ibin)%n_alloc +1
    
                 allocate(obsdiags(i_aaod_ob_type,ibin)%tail%muse(miter+1))
                 allocate(obsdiags(i_aaod_ob_type,ibin)%tail%nldepart(miter+1))
                 allocate(obsdiags(i_aaod_ob_type,ibin)%tail%tldepart(miter))
                 allocate(obsdiags(i_aaod_ob_type,ibin)%tail%obssen(miter))
                 obsdiags(i_aaod_ob_type,ibin)%tail%indxglb=(ioid(n)-1)*nchanl+ii
                 obsdiags(i_aaod_ob_type,ibin)%tail%nchnperobs=-99999
                 obsdiags(i_aaod_ob_type,ibin)%tail%luse=luse(n)
                 obsdiags(i_aaod_ob_type,ibin)%tail%muse(:)=.false.
                 obsdiags(i_aaod_ob_type,ibin)%tail%nldepart(:)=-huge(zero)
                 obsdiags(i_aaod_ob_type,ibin)%tail%tldepart(:)=zero
                 obsdiags(i_aaod_ob_type,ibin)%tail%wgtjo=-huge(zero)
                 obsdiags(i_aaod_ob_type,ibin)%tail%obssen(:)=zero
    
                 n_alloc(ibin) = n_alloc(ibin) +1
                 my_diag => obsdiags(i_aaod_ob_type,ibin)%tail
                 my_diag%idv = is
                 my_diag%iob = ioid(n)
                 my_diag%ich = ii
                 ...
                 if (.not.associated(obsdiags(i_aaod_ob_type,ibin)%tail)) then
                    obsdiags(i_aaod_ob_type,ibin)%tail => obsdiags(i_aaod_ob_type,ibin)%head
                 else
                    obsdiags(i_aaod_ob_type,ibin)%tail => obsdiags(i_aaod_ob_type,ibin)%tail%next
                 end if
                 if (.not.associated(obsdiags(i_aaod_ob_type,ibin)%tail)) then
                    call die(myname,'.not.associated(obsdiags(i_aaod_ob_type,ibin)%tail)')
                 end if
                 if (obsdiags(i_aaod_ob_type,ibin)%tail%indxglb/=(ioid(n)-1)*nchanl+ii) then
                    write(6,*)'setupaod_aaod_ce318: index error'
                    call stop2(278)
                 endif
              endif

              if (in_curbin.and.icc>0) then
                 !my_head => aaodNode_typecast(obsLList_tailNode(aaodhead(ibin)))
                 my_node => obsLList_tailNode(aaodhead(ibin))
                 if(.not.associated(my_node)) &
                    call die(myname,'unexpected, associated(my_node) =',associated(my_node))
                 my_head => aaodNode_typecast(my_node)
                 if(.not.associated(my_head)) &
                    call die(myname,'unexpected, associated(my_head) =',associated(my_head))
                 my_node => null()

                 if (ii==1) obsptr => obsdiags(i_aaod_ob_type,ibin)%tail
                 if (ii==1) obsdiags(i_aaod_ob_type,ibin)%tail%nchnperobs = nchanl
                 obsdiags(i_aaod_ob_type,ibin)%tail%nldepart(jiter) = aod(ii)
                 obsdiags(i_aaod_ob_type,ibin)%tail%wgtjo=varinv(ii)
 
!                Load data into output arrays
                 m=ich(ii)
                 if (varinv(ii)>tiny_r_kind .and. iuse_aero(m)>=1) then
                    iii=iii+1
                    my_head%diags(iii)%ptr => obsdiags(i_aaod_ob_type,ibin)%tail
                    obsdiags(i_aaod_ob_type,ibin)%tail%muse(jiter) = .true.
           ...
           if (in_curbin) then
              if( iii/=icc ) then
                 write(6,*)'setupaod_aaod_ce318: error iii icc',iii,icc
                 call stop2(279)
              endif
           endif ! (in_curbin)
        endif ! (luse_obsdiag)
              ...
              if (l_may_be_passive) then
                 do ii=1,nchanl
                    if (.not.associated(obsptr)) then
                       write(6,*)'setupaod_aaod_ce318: error obsptr'
                       call stop2(280)
                    end if
                    if (obsptr%indxglb/=(ioid(n)-1)*nchanl+ii) then
                       write(6,*)'setupaod_aaod_ce318: error writing diagnostics'
                       call stop2(281)
                    end if
...
! Deallocate arrays
  ...
  if (aero_diagsave) then
     call dtime_show(myname,'diagsave:aaod',i_aaod_ob_type)
     close(4)
  endif
...
end subroutine setupaod_aaod_ce318
