...
contains  
  ...
  ! Declare local parameters
    ...
    character(len=31), dimension(:), allocatable :: c_mosaic_var
    real(r_single),allocatable:: mas_a0x(:,:,:,:)
    !!! cwy mosaic -------------------------------------------------------- chem input
  ...
  ! to temporary binary format
    ...
    n_loop: do n=1,9 ! loop over forecast hours in assim interval
       ...
       write(filename,'("sigf",i2.2)') n                      !!! sigf03
  ...
  !-------------  get grid info
       ...
       if(wrf_mass_hybridcord) then     !!! wrf_mass_hybridcord=F
          rmse_var='C3H'
          call ext_ncd_get_var_info (dh1,trim(rmse_var),ndim1,ordering,staggering, &
            start_index,end_index, WrfType, ierr    )
          if(print_verbose)then
...
!      rmse_var='LANDMASK'
       ...
       call ext_ncd_read_field(dh1,DateStr1,TRIM(rmse_var),              &
            ...
            ierr                                 )
       k=1
       if(l_gsd_soilTQ_nudge) then    !!! l_gsd_...=F
      ...
      !!! --------------------------- l_cloud_nalaysis .and. n_actual_clouds>0
      ...
      !!! cwy mosaic -------------------------------------------------------- chem input
      if( n==3 .and. laeroana_mosaic ) then
        call gsi_chemguess_get('aerosols::3d', n_mosaic_var, ier)  !!! i4crtm>=10 & <20
        allocate( c_mosaic_var(n_mosaic_var) )
        call gsi_chemguess_get('aerosols::3d', c_mosaic_var, ier)  !!! i4crtm>=10 & <20

        allocate( mas_a0x(nsize_mosaic_wrf,nlon_regional,nlat_regional,nsig_regional) )
        mas_a0x=0.
        doiv: do iv = 1, n_mosaic_var
          rmse_var=c_mosaic_var(iv)

          !-------
          if( index(rmse_var,'mas')==0 ) then 
            call ext_ncd_get_var_info(dh1,trim(rmse_var),ndim1,ordering,staggering, &
                                      start_index,end_index, WrfType, ierr    )
            call ext_ncd_read_field(dh1,DateStr1,TRIM(rmse_var),              &    
              field3,WRF_REAL,0,0,0,ordering,           &    
            ...
            if( index(rmse_var,'num')==0 .and. index(rmse_var,'water')==0 ) then
              do k=1, nsig_regional
              do j=1, nlat_regional
              do i=1, nlon_regional
              if( index(rmse_var,'a01')/=0 ) mas_a0x(1,i,j,k)=mas_a0x(1,i,j,k)+field3(i,j,k)
              if( index(rmse_var,'a02')/=0 ) mas_a0x(2,i,j,k)=mas_a0x(2,i,j,k)+field3(i,j,k)
              if( index(rmse_var,'a03')/=0 ) mas_a0x(3,i,j,k)=mas_a0x(3,i,j,k)+field3(i,j,k)
              if( index(rmse_var,'a04')/=0 ) mas_a0x(4,i,j,k)=mas_a0x(4,i,j,k)+field3(i,j,k)
              if( index(rmse_var,'a05')/=0 ) mas_a0x(5,i,j,k)=mas_a0x(5,i,j,k)+field3(i,j,k)
              if( index(rmse_var,'a06')/=0 ) mas_a0x(6,i,j,k)=mas_a0x(6,i,j,k)+field3(i,j,k)
              if( index(rmse_var,'a07')/=0 ) mas_a0x(7,i,j,k)=mas_a0x(7,i,j,k)+field3(i,j,k)
              if( index(rmse_var,'a08')/=0 ) mas_a0x(8,i,j,k)=mas_a0x(8,i,j,k)+field3(i,j,k)
              end do
              end do
              end do
            end if
          !-------
          else if( index(rmse_var,'mas')/=0 ) then 
            !!! cwy: mas_a0x must show at the end of aersol list in chem_guess:: in comgsi_namelist
            do k=1, nsig_regional
            do j=1, nlat_regional
            do i=1, nlon_regional
            if( index(rmse_var,'mas_a01')/=0 ) field3(i,j,k)=mas_a0x(1,i,j,k)
            if( index(rmse_var,'mas_a02')/=0 ) field3(i,j,k)=mas_a0x(2,i,j,k)
            if( index(rmse_var,'mas_a03')/=0 ) field3(i,j,k)=mas_a0x(3,i,j,k)
            if( index(rmse_var,'mas_a04')/=0 ) field3(i,j,k)=mas_a0x(4,i,j,k)
            if( index(rmse_var,'mas_a05')/=0 ) field3(i,j,k)=mas_a0x(5,i,j,k)
            if( index(rmse_var,'mas_a06')/=0 ) field3(i,j,k)=mas_a0x(6,i,j,k)
            if( index(rmse_var,'mas_a07')/=0 ) field3(i,j,k)=mas_a0x(7,i,j,k)
            if( index(rmse_var,'mas_a08')/=0 ) field3(i,j,k)=mas_a0x(8,i,j,k)
            end do
            end do
            end do
          !-------
        ...
        end do doiv ! n_mosaic_var loop
        deallocate( c_mosaic_var, mas_a0x )
  ...
  ! Read in the rest of the fields
    ...
    if(laeroana_mosaic) then 
      ...
      allocate( c_mosaic_var(n_mosaic_var) )
      call gsi_chemguess_get('aerosols::3d', c_mosaic_var, ier) 
      do iv = 1, n_mosaic_var
        ...
        end do ! n_mosaic_var loop
        deallocate( c_mosaic_var )
    endif ! laeroana_mosaic
    !!! cwy mosaic -------------------------------------------------------- chem output
