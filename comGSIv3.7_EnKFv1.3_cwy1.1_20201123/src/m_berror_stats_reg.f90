...
!!! cwy srf_eabs_esca ---------- need to match len_vars_out in getn_be_stage2_gsi.f90
...
!    load the variances
     do n=1,nrf
        if (var==cvars(n)) then
           ...
           loc=n
           !!! -------------------------------------
           print *, 'find be.gcv var='//trim(var)
           !!! -------------------------------------
           exit
        else
           loc=-999
...
! 3d variable
  do n=1,nc3d
     loc=nrf3_loc(n)
     print *, 'handle be.gcv var='//trim(cvars(loc))
     if (nrf_err(loc)) then
        do k=1,nsig
           m=lsig(k)
           m1=m+1
        ...
        !!! ----------------------------------------------
        print *, 'STOP cannot find be.gcv for var='//trim(cvars(loc))
        stop
        !!! ----------------------------------------------
        do k=1,nsig
           do i=1,mlat
