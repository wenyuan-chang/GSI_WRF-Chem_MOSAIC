subroutine read_aerosol_srfesca(nread,ndata,nodata,infile,obstype,gstime,lunout, &
           twind,sis,nobs)
!
...
!!! cwy ----------------------------------------------- wavnum
  real(r_double), dimension( 3) :: srf_esca
!!! cwy ----------------------------------------------- wavnum
...
! Set constants.  Initialize variables
  ...
  if ( obstype == 'srf_esca' ) then
     open(lunin,file=trim(infile),form='unformatted')
!!! cwy ------------------------------------------------- specified table
!!!     open(99,file=trim(infile)//'.table')
!!!     print *, 'cwy srf.esca.bufr.table=', trim(infile)//'.table'
...
!!! cwy ------------------------------------------------- specified table
     ...
     if ( iret == 0 ) then
        if (subset == 'NC008041') then
           write(6,*)'READ_AEROSOL_SRF_ESCA: SRF_ESCA data type, subset = ',subset
           nreal=6
           nchanl=20
           ...
           write(6,'(''READ_AEROSOL_SRFESCA: aerosol bufr file '',a,''  date is '',i4,4i2.2,a)')trim(infile),iy,im,idd,ihh
           ...
           read_loop: do
              ...
              call ufbint(lunin,srf_esca,3,1,iret,aerostr)
...
!!! cwy ----------------------------------------------- wavnum
              do ksw=1, 3
!!! cwy ----------------------------------------------- wavnum
              if ( srf_esca(ksw)>=10000_r_double .or. srf_esca(ksw)<=0. ) cycle read_loop
              end do
...
!!! NOTE: 1-3 for absorbing aod, 4-7 for scattering aod
              dataaod(1) = srf_esca(1)
              dataaod(2) = srf_esca(2)
              dataaod(3) = srf_esca(3)
              print '(a17,3f10.3)', 'cwy obs srf_esca=', dataaod(1), dataaod(2), dataaod(3)
!!! cwy ----------------------------------------------- wavnum

              itx=nread
              if( itx>ce318max ) then
                write(*,*) 'cwy stop error: ce318obs>ce318max in read_aerosol_srfesca.f90'
                stop
              end if
           ...
           write(6,*)'READ_AEROSOL_SRFESCA:  *** WARNING: unknown aerosol data type, subset=',subset
           write(6,*)' infile=',infile, ', lunin=',lunin, ', obstype=',obstype
           write(6,*)' SKIP PROCESSING OF THIS MODIS FILE'
        endif

     else          ! read subset iret /= 0
        write(6,*)'READ_AEROSOL_SRFESCA:  *** WARNING: read subset error, obstype=',obstype,', iret=',iret
     end if
     call closbf(lunin)
     close(lunin)
  else             ! obstype /= 'modis'
     write(6,*)'READ_AEROSOL_SRFESCA:  *** WARNING: unknown aerosol input type, obstype=',obstype
  endif

  ! Deallocate satthin arrays
  call destroygrids

end subroutine read_aerosol_srfesca
