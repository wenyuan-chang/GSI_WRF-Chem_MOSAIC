...
! Set diagnostic output flag
  ...
  conv_diagsave = write_diag(jiter) .and. diag_conv                 !!! diag_conv=T
  ozone_diagsave= write_diag(jiter) .and. diag_ozone .and. ihave_oz
  co_diagsave   = write_diag(jiter) .and. diag_co    .and. ihave_co
  aero_diagsave = write_diag(jiter) .and. diag_aero                 !!! diag_aero=F, write_diag in ush/comgsi_namelist_chem.sh
...
!!! cwy srf_eabs_esca -------------------------------------
!          Set up for surface eabs data
           else if(ditype(is) == 'srf_eabs')then
              call setupaod_srfeabs(lunin,&
                 mype,nchanl,nreal,nobs,&
                 obstype,isis,is,aero_diagsave,init_pass)
!          Set up for surface esca data
           else if(ditype(is) == 'srf_esca')then
              call setupaod_srfesca(lunin,&
                 mype,nchanl,nreal,nobs,&
                 obstype,isis,is,aero_diagsave,init_pass)
!          Set up for surface eext data
           else if(ditype(is) == 'srf_eext')then
              call setupaod_srfeext(lunin,&
                 mype,nchanl,nreal,nobs,&
                 obstype,isis,is,aero_diagsave,init_pass)
!!! cwy srf_eabs_esca -------------------------------------
!!! cwy mosaic ------------------------------------------------------
