...
use crtm_aod_module, only: crtm_aod_k, crtm_aod
!!! add ---------------------------
...
!!!  if (n_actual_aerosols > 0 .and. laeroana_gocart ) then
...
!!!                   layer_od,jacobian_aero)  
                   layer_od,jacobian_aero,aod550_obs)  
!!! add ------------------------------------
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!!! add -----------------------------
  real(r_kind),intent(in)  ,optional :: aod550_obs
!!! add -----------------------------
...
!!! add ------------------------------------------
    do k=1,nsig
        rh(k) = q(k)/( gesqsat(ix , iy,k,1)*w00+ &
                       gesqsat(ixp, iy,k,1)*w10+ &
                       gesqsat(ix ,iyp,k,1)*w01+ &
                       gesqsat(ixp,iyp,k,1)*w11 )
    end do
!!! add ------------------------------------------
  endif
...
!!!     call Set_CRTM_Aerosol ( msig, n_actual_aerosols_wk, n_aerosols_fwd_wk, aerosol_names, aero_conc, auxrh, &
     call Set_CRTM_Aerosol ( msig, n_actual_aerosols_wk, n_aerosols_fwd_wk, aerosol_names, aero_conc, auxrh, atmosphere(1)%temperature, &
...
!!! add -------------------------------------------------------------
     error_status = crtm_aod(atmosphere,&
        channelinfo(sensorindex:sensorindex),rtsolution)
      do i=1, nchanl
!!!       rtsolution_k(i,1)%layer_optical_depth=aod550_obs-rtsolution(i,1)%layer_optical_depth
       rtsolution_k(i,1)%layer_optical_depth=1.
      end do
!!! add -------------------------------------------------------------
