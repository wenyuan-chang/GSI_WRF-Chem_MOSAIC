...
!$$$ end documentation block
  ...
  use m_aodxNode , only: aodxNode
  use m_aodxNode , only: aodxNode_typecast
  use m_aodxNode , only: aodxNode_nextcast
  implicit none

  private
  public :: intaod_aodx

  interface intaod_aodx; module procedure &
            intaod_aodx_
  end interface

contains

  subroutine intaod_aodx_(aerohead,rval,sval)
...
!   real(r_kind) cg_aero,p0,wnotgross,wgross
  ...
  ! Inquire about chemistry
    ...
    !aeroptr => aerohead
    aeroptr => aodxNode_typecast(aerohead)
...
!    Distribute adjoint contributions over surrounding grid points
          ...
          do ic = 1, naero
             ...
             if ( istatus /= 0 ) then
                write(6,*) 'error gsi_bundlegetpointer in intaod_aodx for ', aerojacnames(ic)
             end if
             do k=1,nsig
                n = k + nsig*(ic-1)
                i1 = i1n(k)
       ...
       aeroptr => aodxNode_nextcast(aeroptr)
!       call stop2(999)
    end do

    return
  end subroutine intaod_aodx_

end module intaodmod_aodx
