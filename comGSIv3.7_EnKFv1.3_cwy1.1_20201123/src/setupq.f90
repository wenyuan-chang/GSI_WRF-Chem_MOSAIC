...
! Interpolate log(ps) & log(pres) at mid-layers to obs locations/times
     ...
     presq=r10*exp(dpres)   ! presq in mb=hPa
     itype=ictype(ikx)
     dprpx=zero
...
!!! --------------------------------------------------------
        ...
        if(oberror_tune) then     !!!  oberror_tune=F
...
!!!!!!!!!!!!!!  PBL pseudo surface obs  !!!!!!!!!!!!!!!!
     if( .not. last .and. l_pbl_pseudo_surfobsq .and.         &   !!1  l_pbl_pseudo_surfobsq=F
...
! Write information to diagnostic file
  if(conv_diagsave) then
    if(netcdf_diag) call nc_diag_write                    !!!  netcdf_diag=F
    if(binary_diag .and. ii>0)then
       call dtime_show(myname,'diagsave:q',i_q_ob_type)   !!!  binary_diag=T
       write(7)'  q',nchar,nreal,ii+iip,mype,ioff0
       if(l_pbl_pseudo_surfobsq .and. iip>0) then         !!!  l_pbl_pseudo_surfobsq=F
