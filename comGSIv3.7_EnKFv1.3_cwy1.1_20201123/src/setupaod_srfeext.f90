   subroutine setupaod_srfeext(lunin,mype,nchanl,nreal,nobs,&
...
!$$$
  ...
  use obsmod, only: sf_srf_eext
  use obsmod, only: i_srfeext_ob_type
  use m_obsdiags, only: srfeexthead
  use m_srfeextNode, only: srfeextNode, srfeextNode_typecast
...
! Declare local arrays
  ...
  type(srfeextNode),pointer:: my_head
  type(obs_diag),pointer:: my_diag
  type(rad_obs_type) :: radmod
  character(len=*),parameter:: myname="setupaod_srfeext"
...
!!! cwy mosaic ------------------------------------------------------------------------ chem
  real(r_kind) :: error_1, error_2, dlon, dlat, dlon_earth, dlat_earth
!!!  logical      :: outside
...
!       Set error instrument channels
  ...
  write(*,*) 'cwy: setupaod_srfeext.F90'
...
!    first outer iteration.
     if (init_pass .and. mype==mype_diaghdr(is)) then
        ...
        write(6,*)'setupaod_srfeext:  write header record for ',&
...
!       Interpolate model fields to observation location, call crtm and create jacobians
        ...
        call call_aod_mosaic_srf(dtime,data_s(:,n),nchanl,nreal,srf_od,jacobian_aero,'srf_eext') ! srf_eabs, srf_esca
        else
...
! files are populated, but this could be changed if it causes problems.
        ...
        do i = 1, nchanl
!!! cwy mosaic ------------------------------------------------------------------------
           error_1=aod_obs(i)*0.1
           error_2=0.5*error_1*sqrt(region_dx(nint(dlat),nint(dlon))/15000.)
           tnoise(i)=sqrt(error_1**2+error_2**2)      ! Mm^-1
!!! cwy mosaic ------------------------------------------------------------------------ chem
...
!!! cwy mosaic ------------------------------------------------------------------------ chem
                if( laeroana_mosaic ) then
                  ...
                  !!!tnoise(i)=sf_srf_eext*(10.+0.1*aod_obs(i))
                  tnoise(i)=sf_srf_eext*tnoise(i)

                  error0(i) = tnoise(i)
                  varinv(i) = val_obs/tnoise(i)**2

!!!                  print *, 'cwy dx=', region_dx(nint(dlat),nint(dlon))
!!!                  write(*,'(a20,f10.3,a13,f15.3,a11,f10.3,a3,i1)') &
!!!                  & 'cwy srf eext_obs(i)=',aod_obs(i),' eext_sim(i)=',total_aod(i),' tnoise(i)=',tnoise(i),' i=',i
                end if
!!! cwy mosaic ------------------------------------------------------------------------ chem
...
!!!             write(6,'(A,3i6,f15.3,3f10.3,2f10.3)') 'cwy, mype, iobs, ichan, srf_eext_obs, srf_eext_sim, omb, err, lat, lon : ',  &
...
!          Load data into output arrays
           if (icc > 0) then
              ...
              !!! cwy mosaic -------------------------------------------
              call obsLList_appendNode(srfeexthead(ibin),my_node)
              !!! cwy mosaic -------------------------------------------
              my_node => null()
        ...
        if(luse_obsdiag)then
           ...
           do ii=1,nchanl
              if (.not.lobsdiag_allocated) then
                 if (.not.associated(obsdiags(i_srfeext_ob_type,ibin)%head)) then
                    obsdiags(i_srfeext_ob_type,ibin)%n_alloc = 0
                    allocate(obsdiags(i_srfeext_ob_type,ibin)%head,stat=istat)
                    if (istat/=0) then
                       write(6,*)'setupaod_srfeext: failure to allocate obsdiags',istat
                       call stop2(276)
                    end if
                    obsdiags(i_srfeext_ob_type,ibin)%tail => obsdiags(i_srfeext_ob_type,ibin)%head
                 else
                    allocate(obsdiags(i_srfeext_ob_type,ibin)%tail%next,stat=istat)
                    if (istat/=0) then
                       write(6,*)'setupaod_srfeext: failure to allocate obsdiags',istat
                       call stop2(277)
                    end if
                    obsdiags(i_srfeext_ob_type,ibin)%tail => obsdiags(i_srfeext_ob_type,ibin)%tail%next
                 end if
                 obsdiags(i_srfeext_ob_type,ibin)%n_alloc = obsdiags(i_srfeext_ob_type,ibin)%n_alloc +1
    
                 allocate(obsdiags(i_srfeext_ob_type,ibin)%tail%muse(miter+1))
                 allocate(obsdiags(i_srfeext_ob_type,ibin)%tail%nldepart(miter+1))
                 allocate(obsdiags(i_srfeext_ob_type,ibin)%tail%tldepart(miter))
                 allocate(obsdiags(i_srfeext_ob_type,ibin)%tail%obssen(miter))
                 obsdiags(i_srfeext_ob_type,ibin)%tail%indxglb=(ioid(n)-1)*nchanl+ii
                 obsdiags(i_srfeext_ob_type,ibin)%tail%nchnperobs=-99999
                 obsdiags(i_srfeext_ob_type,ibin)%tail%luse=luse(n)
                 obsdiags(i_srfeext_ob_type,ibin)%tail%muse(:)=.false.
                 obsdiags(i_srfeext_ob_type,ibin)%tail%nldepart(:)=-huge(zero)
                 obsdiags(i_srfeext_ob_type,ibin)%tail%tldepart(:)=zero
                 obsdiags(i_srfeext_ob_type,ibin)%tail%wgtjo=-huge(zero)
                 obsdiags(i_srfeext_ob_type,ibin)%tail%obssen(:)=zero
    
                 n_alloc(ibin) = n_alloc(ibin) +1
                 my_diag => obsdiags(i_srfeext_ob_type,ibin)%tail
                 my_diag%idv = is
                 my_diag%iob = ioid(n)
                 my_diag%ich = ii
                 ...
                 if (.not.associated(obsdiags(i_srfeext_ob_type,ibin)%tail)) then
                    obsdiags(i_srfeext_ob_type,ibin)%tail => obsdiags(i_srfeext_ob_type,ibin)%head
                 else
                    obsdiags(i_srfeext_ob_type,ibin)%tail => obsdiags(i_srfeext_ob_type,ibin)%tail%next
                 end if
                 if (.not.associated(obsdiags(i_srfeext_ob_type,ibin)%tail)) then
                    call die(myname,'.not.associated(obsdiags(i_srfeext_ob_type,ibin)%tail)')
                 end if
                 if (obsdiags(i_srfeext_ob_type,ibin)%tail%indxglb/=(ioid(n)-1)*nchanl+ii) then
                    write(6,*)'setupaod_srfeext: index error'
                    call stop2(278)
                 endif
              endif
              ...
              if (in_curbin.and.icc>0) then
                 ...
                 my_node => obsLList_tailNode(srfeexthead(ibin))
                 if(.not.associated(my_node)) &
                    call die(myname,'unexpected, associated(my_node) =',associated(my_node))
                 my_head => srfeextNode_typecast(my_node)
                 if(.not.associated(my_head)) &
                    call die(myname,'unexpected, associated(my_head) =',associated(my_head))
                 my_node => null()

                 if (ii==1) obsptr => obsdiags(i_srfeext_ob_type,ibin)%tail
                 if (ii==1) obsdiags(i_srfeext_ob_type,ibin)%tail%nchnperobs = nchanl
                 obsdiags(i_srfeext_ob_type,ibin)%tail%nldepart(jiter) = aod(ii)
                 obsdiags(i_srfeext_ob_type,ibin)%tail%wgtjo=varinv(ii)
 
!                Load data into output arrays
                 m=ich(ii)
                 if (varinv(ii)>tiny_r_kind .and. iuse_aero(m)>=1) then
                    iii=iii+1
                    my_head%diags(iii)%ptr => obsdiags(i_srfeext_ob_type,ibin)%tail
                    obsdiags(i_srfeext_ob_type,ibin)%tail%muse(jiter) = .true.
           ...
           if (in_curbin) then
              if( iii/=icc ) then
                 write(6,*)'setupaod_srfeext: error iii icc',iii,icc
                 call stop2(279)
              endif
           endif ! (in_curbin)
        endif ! (luse_obsdiag)
              ...
              if (l_may_be_passive) then
                 do ii=1,nchanl
                    if (.not.associated(obsptr)) then
                       write(6,*)'setupaod_srfeext: error obsptr'
                       call stop2(280)
                    end if
                    if (obsptr%indxglb/=(ioid(n)-1)*nchanl+ii) then
                       write(6,*)'setupaod_srfeext: error writing diagnostics'
                       call stop2(281)
                    end if
...
! Deallocate arrays
  ...
  if (aero_diagsave) then
     call dtime_show(myname,'diagsave:aero',i_srfeext_ob_type)
     close(4)
  endif
...
end subroutine setupaod_srfeext
