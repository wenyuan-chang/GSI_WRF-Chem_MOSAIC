   subroutine setupaod_srfeabs(lunin,mype,nchanl,nreal,nobs,&
...
!$$$
  ...
  use obsmod, only: sf_srf_eabs
  use obsmod, only: i_srfeabs_ob_type
  use m_obsdiags, only: srfeabshead
  use m_srfeabsNode, only: srfeabsNode, srfeabsNode_typecast
  !!! cwy mosaic --------------------------
...
! Declare local arrays
  ...
  type(srfeabsNode),pointer:: my_head
  type(obs_diag),pointer:: my_diag
  type(rad_obs_type) :: radmod
  character(len=*),parameter:: myname="setupaod_srfeabs"
...
!!!  if ( .not. laeroana_gocart ) then
  if ( .not.(laeroana_gocart .or. laeroana_mosaic) ) then
     return
  endif
!!! cwy mosaic ------------------------------------------------------------------------ chem
...
! Initialize channel related information
  ...
  do j=1,jpch_aero
     if(isis == nusis_aero(j))then 
        jc=jc+1
        if(jc > nchanl)then
           write(6,*)'setupaod_srfeabs:  ***ERROR*** in channel numbers, jc,nchanl=',jc,nchanl,&
                '  ***STOP IN setupaod_srfeabs***'
           call stop2(71)
        end if
...
!       Set error instrument channels
  ...
  if ( mype == 0 .and. .not.l_may_be_passive) write(6,*)mype,'setupaod_srfeabs: passive obs',is,isis
  if(nchanl > jc) write(6,*)'setupaod_srfeabs:  channel number reduced for ', &
       obstype,nchanl,' --> ',jc
  if(jc == 0) then
     if(mype == 0) write(6,*)'setupaod_srfeabs: No channels found for ', &
          obstype,isis
     if(nobs > 0)read(lunin)
     return
  end if
  if (toss) then
     if(mype == 0)write(6,*)'setupaod_srfeabs: all obs var > 1e4.  do not use ',&
  ...
  write(*,*) 'cwy: setupaod_srfeabs.F90'
...
!    first outer iteration.
     if (init_pass .and. mype==mype_diaghdr(is)) then
        ...
        write(6,*)'setupaod_srfeabs:  write header record for ',&
...
!       Load channel data into work array.
        ...
        write(*,*) 'l_aoderr_table=', l_aoderr_table
...
!       Interpolate model fields to observation location, call crtm and create jacobians
        ...
        call call_aod_mosaic_srf(dtime,data_s(:,n),nchanl,nreal,srf_od,jacobian_aero,'srf_eabs') ! srf_eabs, srf_esca
        else
...
!!! cwy mosaic ------------------------------------------------------------------------ chem
           tnoise(i)=10.    ! Mm^-1
!!!          tnoise(i)=10.+aod_obs(i)*0.01    ! Mm^-1
!!! cwy mosaic ------------------------------------------------------------------------ chem
...
!!! cwy mosaic ------------------------------------------------------------------------ chem
                if( laeroana_mosaic ) then
                  ...
                  !!!tnoise(i)=sf_srf_eabs*(10.+0.1*aod_obs(i))
                  tnoise(i)=sf_srf_eabs*tnoise(i)

                  error0(i) = tnoise(i)
                  varinv(i) = val_obs/tnoise(i)**2

!!!                  write(*,'(a20,f10.3,a13,f15.3,a11,f10.3,a3,i1)') &
!!!                  & 'cwy srf eabs_obs(i)=',aod_obs(i),' eabs_sim(i)=',total_aod(i),' tnoise(i)=',tnoise(i),' i=',i
                end if
!!! cwy mosaic ------------------------------------------------------------------------ chem
...
!!!             write(6,'(A,3i6,f15.3,3f10.3,2f10.3)') 'cwy, mype, iobs, ichan, srf_eabs_obs, srf_eabs_sim, omb, err, lat, lon : ',  &
...
!          Load data into output arrays
           if (icc > 0) then
              ...
              !!! cwy mosaic -------------------------------------------
              call obsLList_appendNode(srfeabshead(ibin),my_node)
              !!! cwy mosaic -------------------------------------------
              my_node => null()
        ...
        if(luse_obsdiag)then
           ...
           do ii=1,nchanl
              if (.not.lobsdiag_allocated) then
                 if (.not.associated(obsdiags(i_srfeabs_ob_type,ibin)%head)) then
                    obsdiags(i_srfeabs_ob_type,ibin)%n_alloc = 0
                    allocate(obsdiags(i_srfeabs_ob_type,ibin)%head,stat=istat)
                    if (istat/=0) then
                       write(6,*)'setupaod_srfeabs: failure to allocate obsdiags',istat
                       call stop2(276)
                    end if
                    obsdiags(i_srfeabs_ob_type,ibin)%tail => obsdiags(i_srfeabs_ob_type,ibin)%head
                 else
                    allocate(obsdiags(i_srfeabs_ob_type,ibin)%tail%next,stat=istat)
                    if (istat/=0) then
                       write(6,*)'setupaod_srfeabs: failure to allocate obsdiags',istat
                       call stop2(277)
                    end if
                    obsdiags(i_srfeabs_ob_type,ibin)%tail => obsdiags(i_srfeabs_ob_type,ibin)%tail%next
                 end if
                 obsdiags(i_srfeabs_ob_type,ibin)%n_alloc = obsdiags(i_srfeabs_ob_type,ibin)%n_alloc +1
    
                 allocate(obsdiags(i_srfeabs_ob_type,ibin)%tail%muse(miter+1))
                 allocate(obsdiags(i_srfeabs_ob_type,ibin)%tail%nldepart(miter+1))
                 allocate(obsdiags(i_srfeabs_ob_type,ibin)%tail%tldepart(miter))
                 allocate(obsdiags(i_srfeabs_ob_type,ibin)%tail%obssen(miter))
                 obsdiags(i_srfeabs_ob_type,ibin)%tail%indxglb=(ioid(n)-1)*nchanl+ii
                 obsdiags(i_srfeabs_ob_type,ibin)%tail%nchnperobs=-99999
                 obsdiags(i_srfeabs_ob_type,ibin)%tail%luse=luse(n)
                 obsdiags(i_srfeabs_ob_type,ibin)%tail%muse(:)=.false.
                 obsdiags(i_srfeabs_ob_type,ibin)%tail%nldepart(:)=-huge(zero)
                 obsdiags(i_srfeabs_ob_type,ibin)%tail%tldepart(:)=zero
                 obsdiags(i_srfeabs_ob_type,ibin)%tail%wgtjo=-huge(zero)
                 obsdiags(i_srfeabs_ob_type,ibin)%tail%obssen(:)=zero
    
                 n_alloc(ibin) = n_alloc(ibin) +1
                 my_diag => obsdiags(i_srfeabs_ob_type,ibin)%tail
                 my_diag%idv = is
                 my_diag%iob = ioid(n)
                 my_diag%ich = ii
                 ...
                 if (.not.associated(obsdiags(i_srfeabs_ob_type,ibin)%tail)) then
                    obsdiags(i_srfeabs_ob_type,ibin)%tail => obsdiags(i_srfeabs_ob_type,ibin)%head
                 else
                    obsdiags(i_srfeabs_ob_type,ibin)%tail => obsdiags(i_srfeabs_ob_type,ibin)%tail%next
                 end if
                 if (.not.associated(obsdiags(i_srfeabs_ob_type,ibin)%tail)) then
                    call die(myname,'.not.associated(obsdiags(i_srfeabs_ob_type,ibin)%tail)')
                 end if
                 if (obsdiags(i_srfeabs_ob_type,ibin)%tail%indxglb/=(ioid(n)-1)*nchanl+ii) then
                    write(6,*)'setupaod_srfeabs: index error'
                    call stop2(278)
                 endif
              endif
              ...
              if (in_curbin.and.icc>0) then
                 ...
                 my_node => obsLList_tailNode(srfeabshead(ibin))
                 if(.not.associated(my_node)) &
                    call die(myname,'unexpected, associated(my_node) =',associated(my_node))
                 my_head => srfeabsNode_typecast(my_node)
                 if(.not.associated(my_head)) &
                    call die(myname,'unexpected, associated(my_head) =',associated(my_head))
                 my_node => null()

                 if (ii==1) obsptr => obsdiags(i_srfeabs_ob_type,ibin)%tail
                 if (ii==1) obsdiags(i_srfeabs_ob_type,ibin)%tail%nchnperobs = nchanl
                 obsdiags(i_srfeabs_ob_type,ibin)%tail%nldepart(jiter) = aod(ii)
                 obsdiags(i_srfeabs_ob_type,ibin)%tail%wgtjo=varinv(ii)
 
!                Load data into output arrays
                 m=ich(ii)
                 if (varinv(ii)>tiny_r_kind .and. iuse_aero(m)>=1) then
                    iii=iii+1
                    my_head%diags(iii)%ptr => obsdiags(i_srfeabs_ob_type,ibin)%tail
                    obsdiags(i_srfeabs_ob_type,ibin)%tail%muse(jiter) = .true.
           ...
           if (in_curbin) then
              if( iii/=icc ) then
                 write(6,*)'setupaod_srfeabs: error iii icc',iii,icc
                 call stop2(279)
              endif
           endif ! (in_curbin)
        endif ! (luse_obsdiag)
              ...
              if (l_may_be_passive) then
                 do ii=1,nchanl
                    if (.not.associated(obsptr)) then
                       write(6,*)'setupaod_srfeabs: error obsptr'
                       call stop2(280)
                    end if
                    if (obsptr%indxglb/=(ioid(n)-1)*nchanl+ii) then
                       write(6,*)'setupaod_srfeabs: error writing diagnostics'
                       call stop2(281)
                    end if
...
! Deallocate arrays
  ...
  if (aero_diagsave) then
     call dtime_show(myname,'diagsave:aero',i_srfeabs_ob_type)
     close(4)
  endif
...
end subroutine setupaod_srfeabs
