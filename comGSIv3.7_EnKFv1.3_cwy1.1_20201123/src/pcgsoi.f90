...
!  use guess_grids, only: wt_mas_a01, wt_mas_a02, wt_mas_a03, wt_mas_a04
!  use constants, only: half, one, fv, eps, psv_a, psv_b, psv_c, psv_d
...
!!!! cwy mosaic ----------------------------- chem
!  integer(i_kind) :: ksize, k, j, it, istatus
!  real(r_kind) :: wt_mas_a0x, fact, pw, tmp_K, prs_sv, prs_a, prs_v, rdensity, rl_hm
!  real(r_kind) :: rso4, rno3, rclx, rocx, rbcx, roin
!  character(len=2) :: c2
!  real(r_kind),dimension(lat2,lon2,nsig),target :: mas_a0x
...
! Allocate required memory and initialize fields
  call init_                                      !!! xhat=0., gradx=0, grady=0., sval, mval, ...
  ...
  if(iorthomax>0) then !!! iorthomax=0
...
!!! -------- copy chem & met guess into work bundle ----
...
!       Convert to control space directly from physical space.
        if (l_hyb_ens) then
          ...
          !!! --------------------------------------------------------
           ...
           if (nobs_bins > 1 ) then
              do ii=2,nobs_bins
                 call self_add(mval(1),rval(ii))  !!! mval=mval+rval, gsi_bundlemode.f90
              enddo
           end if
          !!! --------------------------------------------------------
        end if
     ...
     !!! -------- control2state adjoint -------------------------------
     call control2state_ad(mval,rbias,gradx)      !!! mval -> gradx
!    End adjoint of convert control var to physical space
     !!! -------- control2state adjoint -------------------------------

!    Print initial Jo table     !!! print_diag_pcg=F, luse_obsdiag=F
...
!       save gradients
     ...
     if (iter==0 .and. print_diag_pcg) then     !!! print_diag_pcg=F
...
!    Calculate new norm of gradients
     ...
     if(diag_precon)then                    !!! diag_precon=T
        if (lanlerr) then                   !!1 lanlerr=F, local var
...
! Update contributions of incremental values from current outer loop
  ...
  if(print_diag_pcg)then      !!! print_diag_pcg=F
...
!!! ----------
  write(*,*) 'prt_state_norms ------------------------------------ pcgsoi.f90'
